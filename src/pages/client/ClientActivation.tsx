...
import {
  Wifi, Eye, EyeOff, Loader2, CheckCircle2, Clock, Zap,
...
const ClientActivation = () => {
  ...
  // Fetch the latest activation request for this client
  const { data: latestRequest, isLoading } = useQuery({
    queryKey: ["client-activation-request", user?.id],
    queryFn: async () => {
  ...
  useEffect(() => {
    ...
    const channel = supabase
      ...
      .on(
        ...
        () => {
          queryClient.invalidateQueries({ queryKey: ["client-activation-request", user.id] });
        }
      )
      .subscribe();
    return () => {
    ...
    <ClientLayout>
      <div className="space-y-6 max-w-3xl">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-3xl font-bold text-slate-900">Activation des services</h1>
            <p className="text-slate-500 mt-1">
              Demandez l'activation de votre service WiFi en quelques clics
            </p>
          </div>
        </div>
        ...
        {/* Status tracker (existing or active request) */}
              ...
              {awaitingConfirmation && (
                <div className="bg-emerald-50 border border-emerald-300 rounded-lg p-4 space-y-3">
                  <p className="text-sm font-semibold text-emerald-900">
                    ✅ Votre service a été activé! Confirmez-vous que tout fonctionne?
                  </p>
            ...
            <CardContent className="p-6">
              <div className="flex items-center gap-3 mb-3">
                ...
                <span className="text-sm text-slate-500">
                  Dernière demande: {latestRequest.wifi_network_name}
                </span>
              </div>
            ...
            <CardContent className="p-6">
              <form onSubmit={handleSubmit} className="space-y-4">
                <div className="flex items-center gap-2 mb-2 pb-3 border-b">
                  ...
                  <h2 className="text-lg font-bold text-slate-900">Activation WiFi</h2>
                </div>
                ...
                <div>
                  ...
                  <p className="text-xs text-slate-500 mt-1">
                    Au cas où nous devrions vous joindre durant l'activation
                  </p>
                </div>
