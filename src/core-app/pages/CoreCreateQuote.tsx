/**
 * Core Admin Create Quote — Multi-step quote builder (same as Employee but with Core navigation + admin source).
 * Supports: existing client (pre-selected via ?clientId=) + prospect mode.
 */
...
import { ArrowLeft, ArrowRight, Check, Plus, Trash2, Search, User, ShoppingCart, MessageSquare, Eye, UserPlus } from "lucide-react";
...
export default function CoreCreateQuote() {
  ...
  const { data: preselectedProfile } = useQuery({
    ...
    queryFn: async () => {
      const { data } = await supabase
        ...
        .eq("user_id", preselectedClientId!)
        .maybeSingle();
      return data;
    },
  });
  ...
  useEffect(() => {
    if (preselectedProfile && !selectedClient) {
      setSelectedClient(preselectedProfile);
    }
  }, [preselectedProfile]);

  const { data: clients } = useQuery({
    queryKey: ["core-quote-client-search", clientSearch],
    enabled: clientSearch.length >= 2 && !isProspect,
    queryFn: async () => {
      const { data } = await supabase
        .from("profiles")
        .select("user_id, full_name, email, phone")
  ...
  const canProceedFromStep1 = isProspect
    ? (prospectName.trim().length > 0 && prospectEmail.trim().length > 0)
    : !!selectedClient;

  const clientDisplayName = isProspect ? prospectName : selectedClient?.full_name;
  const clientDisplayEmail = isProspect ? prospectEmail : selectedClient?.email;

  const handleSubmit = async (asDraft: boolean) => {
    if (!isProspect && !selectedClient) return;
    if (isProspect && (!prospectName.trim() || !prospectEmail.trim())) return;
    ...
    try {
      ...
      const quote = await createQuoteDraft({
        customerUserId: isProspect ? undefined : selectedClient.user_id,
        sourcePortal: "core",
      ...
      if (!asDraft) {
        // Core admin can directly approve their own quotes
        await updateQuoteStatus(quote.id, "approved", session.user.id, "admin", "Approuvée directement par l'admin");
      }
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-6">
      ...
      {/* STEP 1: Client / Prospect */}
      {step === 1 && (
        <Card>
          <CardHeader>
            <CardTitle className="flex items-center justify-between">
              Sélectionner un client ou prospect
              <Button
                variant={isProspect ? "default" : "outline"}
                size="sm"
                onClick={() => { setIsProspect(!isProspect); if (!isProspect) setSelectedClient(null); }}
              >
                <UserPlus className="h-3.5 w-3.5 mr-1" />
                {isProspect ? "Mode prospect" : "Nouveau prospect"}
              </Button>
            </CardTitle>
          </CardHeader>
          <CardContent className="space-y-4">
            {isProspect ? (
              <div className="space-y-3">
                <div>
                  ...
                  <Input value={prospectName} onChange={(e) => setProspectName(e.target.value)} placeholder="Nom du prospect" />
                </div>
                <div>
                  <label className="text-xs font-medium text-muted-foreground mb-1 block">Courriel *</label>
                  <Input type="email" value={prospectEmail} onChange={(e) => setProspectEmail(e.target.value)} placeholder="email@exemple.com" />
                </div>
                <div>
                  <label className="text-xs font-medium text-muted-foreground mb-1 block">Téléphone</label>
                  <Input value={prospectPhone} onChange={(e) => setProspectPhone(e.target.value)} placeholder="(555) 555-5555" />
                </div>
              </div>
            ) : (
              <>
                <div className="relative">
                  ...
                  <Input placeholder="Nom, courriel ou téléphone..." value={clientSearch} onChange={(e) => setClientSearch(e.target.value)} className="pl-9" />
                </div>
                {selectedClient ? (
                  ...
                  <div className="space-y-1">
                    {(clients || []).map((c: any) => (
                      <button key={c.user_id} onClick={() => { setSelectedClient(c); setClientSearch(""); }} className="w-full text-left p-3 rounded-lg hover:bg-muted/50 transition-colors">
            ...
            <div className="flex justify-end">
              <Button disabled={!canProceedFromStep1} onClick={() => setStep(2)}>
              ...
              <span>
                ...
                {clientDisplayName && (
                  <Badge variant="outline" className="ml-2 text-[10px] font-normal">
                    {isProspect ? "Prospect" : "Client"}: {clientDisplayName}
                  </Badge>
                )}
              </span>
          ...
          <CardContent className="space-y-4">
            <div>
              ...
              <div className="flex flex-wrap gap-1.5">
                {(services || []).map((s: any) => (
                  <button key={s.id} onClick={() => addCatalogService(s)} className="px-2.5 py-1 rounded-md border border-border text-xs hover:bg-primary/10 hover:border-primary/30 transition-colors">
                ...
                {lines.map((line, idx) => (
                  <div key={idx} className="flex items-center gap-2 p-2 rounded-lg border border-border bg-muted/30">
                    {line.line_type === "manual_fee" ? (
                      <Input value={line.label} onChange={(e) => updateLine(idx, "label", e.target.value)} placeholder="Description du frais" className="flex-1 h-8 text-sm" />
                    ) : (
                      <span className="flex-1 text-sm font-medium">{line.label}</span>
                    )}
                    <Input type="number" value={line.quantity} onChange={(e) => updateLine(idx, "quantity", parseInt(e.target.value) || 1)} className="w-16 h-8 text-sm text-center" />
                    <span className="text-xs text-muted-foreground">×</span>
                    <Input type="number" step="0.01" value={line.unit_price} onChange={(e) => updateLine(idx, "unit_price", parseFloat(e.target.value) || 0)} className="w-24 h-8 text-sm text-right" />
                    <span className="text-xs text-muted-foreground">$</span>
            ...
            <div className="flex justify-between pt-2">
              <Button variant="ghost" onClick={() => setStep(1)}><ArrowLeft className="h-4 w-4 mr-1" /> Retour</Button>
              <Button disabled={lines.length === 0} onClick={() => setStep(3)}>Suivant <ArrowRight className="h-4 w-4 ml-1" /></Button>
            </div>
          </CardContent>
        </Card>
      )}
      ...
      {step === 3 && (
        <Card>
          ...
          <CardContent className="space-y-4">
            ...
            <div>
              <p className="text-xs font-medium text-muted-foreground mb-2">Ajouter un rabais (approbation automatique admin)</p>
              ...
              {adjustments.map((a, i) => (
                <div key={i} className="flex items-center justify-between mt-2 p-2 rounded border border-border">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <Badge variant="default" className="text-[10px]">Admin — auto-approuvé</Badge>
            ...
            <div className="flex justify-between pt-2">
              <Button variant="ghost" onClick={() => setStep(2)}><ArrowLeft className="h-4 w-4 mr-1" /> Retour</Button>
              <Button onClick={() => setStep(4)}>Réviser <ArrowRight className="h-4 w-4 ml-1" /></Button>
            </div>
          </CardContent>
        </Card>
      )}
          ...
          <CardContent className="space-y-4">
            <div className="p-3 rounded-lg bg-muted/50">
              <div className="flex items-center gap-2 mb-1">
                <p className="text-xs text-muted-foreground">{isProspect ? "Prospect" : "Client"}</p>
                {isProspect && <Badge variant="outline" className="text-[10px]">Prospect</Badge>}
              </div>
              <p className="font-medium">{clientDisplayName}</p>
              <p className="text-xs text-muted-foreground">{clientDisplayEmail}</p>
            </div>
            <div>
              ...
              {lines.map((l, i) => (
                <div key={i} className="flex justify-between py-1 text-sm border-b border-border last:border-0">
                  ...
                  <span className="font-medium">{(l.unit_price * l.quantity).toFixed(2)} $ / {l.billing_frequency === "monthly" ? "mois" : "unique"}</span>
                </div>
              ))}
            </div>
            {adjustments.length > 0 && (
              <div>
                <p className="text-xs font-medium text-muted-foreground mb-2">Ajustements</p>
                {adjustments.map((a, i) => (
            ...
            <div className="p-3 rounded-lg bg-muted/50 space-y-1">
              <div className="flex justify-between text-sm"><span>Mensuel</span><span className="font-medium">{monthlyTotal.toFixed(2)} $ /mois</span></div>
              <div className="flex justify-between text-sm"><span>Frais uniques</span><span className="font-medium">{oneTimeTotal.toFixed(2)} $</span></div>
              {adjTotal > 0 && <div className="flex justify-between text-sm text-destructive"><span>Rabais</span><span>-{adjTotal.toFixed(2)} $</span></div>}
            ...
            <div className="flex justify-between pt-2">
              <Button variant="ghost" onClick={() => setStep(3)}><ArrowLeft className="h-4 w-4 mr-1" /> Retour</Button>
              <div className="flex gap-2">
                <Button variant="outline" onClick={() => handleSubmit(true)} disabled={isSubmitting}>Sauvegarder brouillon</Button>
                <Button onClick={() => handleSubmit(false)} disabled={isSubmitting}>
                  <Check className="h-4 w-4 mr-1" /> Créer et approuver
                </Button>
              </div>
            </div>
          </CardContent>
