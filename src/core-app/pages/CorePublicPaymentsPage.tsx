    ...
    try {
      const { error } = await supabase.from("email_queue").insert({
        ...
        template_vars: {
          ...
          subject: `Reçu de paiement Nivra — ${row.nivra}`,
          heading: "Voici votre reçu de paiement",
          body_text: `Référence Nivra : ${row.nivra}\nFacture : ${row.invoice}\nMontant : ${fmt(row.amount)}\n\nCliquez pour consulter votre reçu Square officiel.`,
          cta_label: "Voir mon reçu",
          cta_url: row.receipt,
        },
        status: "queued",
        attempts: 0,
