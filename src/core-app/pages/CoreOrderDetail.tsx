/**
 * Nivra Core — Order Detail (ops-grade, dual-mode)
 * 
 * MODE 1: "overview" — Dark read-only summary (existing Core view)
 * MODE 2: "process"  — Full operational workspace, reusing useOrderProcessing
 *                       + existing admin processing components with dark CSS wrapper
 * 
 * Zero duplicated business logic. All mutations come from useOrderProcessing.
 */
...
/* ─── Overview data hook (read-only, same as before) ─── */
function useOrderOverviewData(orderId: string | undefined) {
  return useQuery({
    ...
    queryFn: async () => {
      ...
      const [
        ...
        supabase.from("billing_invoices").select("*").eq("order_id", orderId!).order("created_at", { ascending: false }),
        supabase.from("billing_subscriptions").select("*").eq("order_id", orderId!),
        supabase.from("appointments").select("*").eq("order_id", orderId!).maybeSingle(),
        supabase.from("activity_logs").select("*").eq("entity_type", "order").eq("entity_id", orderId!).order("created_at", { ascending: false }).limit(20),
        supabase.from("contracts").select("id, contract_number, status, is_signed, created_at, client_signed_at, admin_signed_at").eq("order_id", orderId!).order("created_at", { ascending: false }).limit(1).maybeSingle(),
      ]);

      const invoiceList = invoices || [];
      const { data: payments } = invoiceList.length > 0
        ? await supabase.from("billing_payments").select("*").in("invoice_id", invoiceList.map((i: any) => i.id)).order("created_at", { ascending: false })
        : { data: [] as any[] };

      return {
        order, profile, account,
        items: items || [], invoices: invoiceList,
        payments: payments || [], subscriptions: subscriptions || [],
        appointment, activityLogs: activityLogs || [], contract,
      };
    },
  });
}
    ...
    <div className="space-y-4">
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-1.5">
          <button
            ...
            className={`flex items-center gap-1.5 rounded-lg border px-3 py-1.5 text-[11px] font-medium transition-colors ${
              viewMode === "overview"
                ...
                : "border-[hsl(220,15%,18%)] bg-[hsl(220,20%,13%)] text-[hsl(220,10%,50%)] hover:text-white hover:border-[hsl(220,15%,25%)]"
            }`}
          >
          ...
          >
            <Wrench className="h-3.5 w-3.5" /> Traiter
          </button>
        </div>
      </div>
      ...
      {/* ═══ CONTENT ═══ */}
      {viewMode === "process" ? (
        <CoreProcessingMode orderId={orderId} />
      ) : (
        <CoreOverviewMode orderId={orderId} />
      )}
    </div>
  );
};
...
/* ═══════════════════════════════════════════════════════════
   PROCESS MODE — Reuses useOrderProcessing + admin components
   ═══════════════════════════════════════════════════════════ */
function CoreProcessingMode({ orderId }: { orderId: string }) {
  ...
  if (proc.isLoading) {
    return (
      <div className="flex items-center justify-center h-96">
        ...
        <span className="ml-3 text-xs text-[hsl(220,10%,50%)]">Chargement du workspace…</span>
      </div>
    );
  }
  ...
  const clientName = proc.profile?.full_name
    || [order.client_first_name, order.client_last_name].filter(Boolean).join(" ")
    || "—";

  return (
    <div className="core-dark-processing space-y-4">
      {/* ═══ Core-native Header ═══ */}
      <div className="rounded-lg border border-[hsl(220,15%,16%)] bg-[hsl(220,20%,11%)] p-4">
        <div className="flex items-start justify-between">
          <div className="flex items-start gap-3">
            <div className="h-10 w-10 rounded-lg bg-[hsl(220,15%,16%)] flex items-center justify-center shrink-0">
            ...
            <div>
              ...
              <p className="text-[hsl(220,10%,50%)] text-xs mt-0.5">
                {order.service_type || "—"} · {clientName}
              </p>
            </div>
          </div>
        ...
        {/* Quick info row */}
        <div className="flex flex-wrap gap-x-5 gap-y-1.5 mt-3 text-[11px] text-[hsl(220,10%,50%)]">
          <span><User className="h-3 w-3 inline mr-1" />{clientName}</span>
          {order.client_email && <span><Mail className="h-3 w-3 inline mr-1" />{order.client_email}</span>}
          <span>Créée {fmtDateTime(order.created_at)}</span>
          {order.fulfillment_type && <span>Fulfillment: {order.fulfillment_type}</span>}
        </div>
      </div>

      {/* ═══ MAIN LAYOUT: Steps | Content | Summary ═══ */}
      <div className="grid grid-cols-1 lg:grid-cols-[220px_1fr_300px] gap-4">
        {/* LEFT: Workflow Steps — reused directly */}
        <WorkflowStepNav
          steps={proc.workflow}
...
/* ═══════════════════════════════════════════════════════════
   OVERVIEW MODE — Read-only dark summary (original Core view)
   ═══════════════════════════════════════════════════════════ */
function CoreOverviewMode({ orderId }: { orderId: string }) {
  const { data, isLoading, refetch } = useOrderOverviewData(orderId);
  ...
  const openPdf = (blob: Blob, title: string, filename: string) => {
    setPdfBlob(blob); setPdfTitle(title); setPdfFilename(filename); setPdfOpen(true);
  };

  const handleViewInvoicePDF = async (invoiceId: string, invoiceNumber: string) => {
    setDocLoading("invoice"); setDocError(null);
    try {
      const result = await generateCanonicalInvoicePDF(supabase, invoiceId);
      if (result.success && result.blob) openPdf(result.blob, `Facture ${invoiceNumber}`, `Facture_${invoiceNumber}.pdf`);
      else { setDocError(result.error || "Erreur"); toast.error(result.error || "Erreur"); }
    } catch (err: any) { setDocError(err.message); toast.error("Erreur facture"); }
    finally { setDocLoading(null); }
  };

  const handleViewContractPDF = async (contractId: string, contractNumber: string) => {
    setDocLoading("contract"); setDocError(null);
    try {
      const result = await generateCanonicalContractPDF(supabase, contractId);
      if (result.success && result.blob) openPdf(result.blob, `Contrat ${contractNumber}`, `Contrat_${contractNumber}.pdf`);
      else { setDocError(result.error || "Erreur"); toast.error(result.error || "Erreur"); }
    } catch (err: any) { setDocError(err.message); toast.error("Erreur contrat"); }
    finally { setDocLoading(null); }
  };

  const handleViewAllDocs = async () => {
    setDocLoading("all"); setDocError(null);
    try {
      const result = await generateOrderDocuments(orderId);
      if (result?.invoice?.blob) openPdf(result.invoice.blob, "Facture", `Facture_${data?.order?.order_number || ""}.pdf`);
      else { setDocError("Impossible de générer"); toast.error("Erreur documents"); }
    } catch (err: any) { setDocError(err.message); toast.error("Erreur génération"); }
    finally { setDocLoading(null); }
  };
  ...
  const clientName = profile?.full_name || [order.client_first_name, order.client_last_name].filter(Boolean).join(" ") || "—";
    ...
    <div className="space-y-4">
      ...
      <div className="rounded-lg border border-[hsl(220,15%,16%)] bg-[hsl(220,20%,11%)] p-4">
        <div className="flex items-start justify-between">
          ...
          <div className="flex items-center gap-2">
            ...
            <StatusBadge label={order.status} variant={statusToVariant(order.status)} size="sm" />
            <button onClick={() => refetch()} className="flex items-center gap-1.5 rounded-lg border border-[hsl(220,15%,18%)] bg-[hsl(220,20%,13%)] px-2.5 py-1.5 text-[11px] text-[hsl(220,10%,50%)] hover:text-white hover:border-emerald-500/30 transition-colors">
              <RefreshCw className="h-3.5 w-3.5" />
            </button>
          </div>
        </div>
              ...
              <tbody>
                {invoices.map((inv: any) => (
                  <tr key={inv.id} className="border-b border-[hsl(220,15%,13%)] last:border-0 hover:bg-[hsl(220,20%,12%)]">
                    <td className="px-3 py-2"><Link to={`/core/invoices/${inv.id}`} className="font-mono text-white hover:text-blue-400">{inv.invoice_number}</Link></td>
                    ...
                    <td className="px-3 py-2"><span className={`tabular-nums font-medium font-mono ${(inv.balance_due ?? 0) > 0 ? "text-red-400" : "text-[hsl(220,10%,40%)]"}`}>{fmtCAD(inv.balance_due)}</span></td>
      ...
      <div className="rounded-lg border border-[hsl(220,15%,16%)] bg-[hsl(220,20%,11%)] p-4">
        ...
        <div className="grid grid-cols-3 gap-2">
          {invoice ? (
            <button onClick={() => handleViewInvoicePDF(invoice.id, invoice.invoice_number)} disabled={docLoading === "invoice"} className="flex items-center gap-2 rounded-lg border border-[hsl(220,15%,18%)] bg-[hsl(220,20%,13%)] p-3 text-left hover:border-blue-500/30 transition-colors disabled:opacity-50">
            ...
            <div className="flex items-center gap-2 rounded-lg border border-[hsl(220,15%,14%)] bg-[hsl(220,20%,10%)] p-3 opacity-50">
              <FileText className="h-5 w-5 text-[hsl(220,10%,30%)]" /><div><p className="text-[11px] font-medium text-[hsl(220,10%,35%)]">Facture</p><p className="text-[10px] text-[hsl(220,10%,25%)]">Non disponible</p></div>
            </div>
          )}
          {contract ? (
            <button onClick={() => handleViewContractPDF(contract.id, contract.contract_number || "")} disabled={docLoading === "contract"} className="flex items-center gap-2 rounded-lg border border-[hsl(220,15%,18%)] bg-[hsl(220,20%,13%)] p-3 text-left hover:border-blue-500/30 transition-colors disabled:opacity-50">
              ...
              <div className="min-w-0">
                ...
                <p className="text-[10px]">{docLoading === "contract" ? <span className="text-amber-400">Génération…</span> : contract.is_signed || contract.status === "fully_signed" ? <span className="text-emerald-400">Signé</span> : <span className="text-amber-400">{contract.status || "En attente"}</span>}</p>
              </div>
            </button>
          ) : (
            <div className="flex items-center gap-2 rounded-lg border border-[hsl(220,15%,14%)] bg-[hsl(220,20%,10%)] p-3 opacity-50">
              <FileBadge className="h-5 w-5 text-[hsl(220,10%,30%)]" /><div><p className="text-[11px] font-medium text-[hsl(220,10%,35%)]">Contrat</p><p className="text-[10px] text-[hsl(220,10%,25%)]">Non disponible</p></div>
            </div>
          )}
          <button onClick={handleViewAllDocs} disabled={!!docLoading} className="flex items-center gap-2 rounded-lg border border-[hsl(220,15%,18%)] bg-[hsl(220,20%,13%)] p-3 text-left hover:border-blue-500/30 transition-colors disabled:opacity-50">
            <Package className="h-5 w-5 text-blue-400 flex-shrink-0" />
            <div><p className="text-[11px] font-medium text-white">Sommaire</p><p className="text-[10px] text-[hsl(220,10%,40%)]">Tous les documents</p><p className="text-[10px] text-emerald-400">{docLoading === "all" ? "Génération…" : "Générer"}</p></div>
          </button>
        </div>
      </div>
      ...
      {activityLogs.length > 0 && (
        <div className="rounded-lg border border-[hsl(220,15%,16%)] bg-[hsl(220,20%,11%)] overflow-hidden">
          <div className="px-3 py-2.5 border-b border-[hsl(220,15%,16%)]"><p className="text-xs font-semibold text-white">Historique d'activité</p></div>
            ...
            {activityLogs.map((log: any) => (
              <div key={log.id} className="px-3 py-2 border-b border-[hsl(220,15%,13%)] last:border-0 flex items-start gap-3">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  {log.changed_field && <p className="text-[11px] text-[hsl(220,10%,40%)]">{log.changed_field}: <span className="text-red-400 line-through">{log.old_value}</span> → <span className="text-emerald-400">{log.new_value}</span></p>}
      ...
      <PDFViewerDialog open={pdfOpen} onOpenChange={setPdfOpen} pdfBlob={pdfBlob} title={pdfTitle} filename={pdfFilename} />
    </div>
  );
}
