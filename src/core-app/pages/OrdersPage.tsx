 ...
 * Dark ops-grade visual style, routed at /core/orders.
 */
import { useState, useMemo } from "react";
import { useAdminOrders, type AdminOrder } from "@/hooks/admin/useAdminOrders";
...
const WorkQueuePage = () => {
  ...
  return (
    <div className="space-y-5">
      {/* Header */}
      <div>
        <h1 className="text-xl font-semibold text-white">Commandes</h1>
        <p className="text-[13px] text-[hsl(220,10%,50%)] mt-0.5">
          Gestion des commandes Nivra — {counts.total} commande{counts.total !== 1 ? "s" : ""}
        </p>
      </div>
      ...
      <div className="grid grid-cols-4 gap-3">
        {[
          ...
          { label: "Complétées", value: counts.completed },
        ].map(k => (
            ...
            {isLoading ? (
              ...
              <tr>
                <td colSpan={9} className="text-center py-10 text-[hsl(220,10%,35%)] text-[13px]">
                  {search || statusFilter ? "Aucune commande ne correspond aux filtres." : "Aucune commande trouvée."}
                </td>
              </tr>
            ) : (
              filtered.map(o => (
                <tr key={o.id} className="border-b border-[hsl(220,15%,14%)] last:border-0 hover:bg-[hsl(220,15%,13%)] transition-colors">
                  ...
                  <td className="px-4 py-3">
                    <div className="max-w-[180px]">
                      <p className="text-white truncate text-[13px]">{o.client_full_name || "—"}</p>
                      {o.account_number && (
                        <p className="text-[11px] text-[hsl(220,10%,40%)] font-mono">#{o.account_number}</p>
                      )}
                    </div>
                  </td>
                  <td className="px-4 py-3 text-[hsl(220,10%,60%)]">
                    {o.service_type || "—"}
                  </td>
                  <td className="px-4 py-3">
                  ...
                  <td className="px-4 py-3">
                    {o.payment_status ? (
                      <StatusBadge label={o.payment_status} variant={statusToVariant(o.payment_status)} size="sm" />
                    ) : (
                  ...
                  <td className="px-4 py-3">
                    {o.invoice_number ? (
                      <span className="font-mono text-[12px] text-emerald-400">#{o.invoice_number}</span>
                    ) : (
                      <span className="text-[hsl(220,10%,30%)]">—</span>
                    )}
                  </td>
                  <td className="px-4 py-3 tabular-nums text-[hsl(220,10%,70%)]">
                    {o.total_amount != null
