  ...
  return (
    <>
      {Array.from({ length: count }).map((_, i) => (
        <div key={i} className="flex items-center gap-3 px-4 py-3 border-b border-[hsl(220,15%,14%)] last:border-0">
          <div className="flex-1 space-y-1.5">
            <div className="h-3.5 w-28 rounded bg-[hsl(220,15%,14%)]" />
            <div className="h-3 w-48 rounded bg-[hsl(220,15%,12%)]" />
          </div>
          <div className="h-5 w-16 rounded-full bg-[hsl(220,15%,14%)]" />
        </div>
      ))}
    </>
  );
...
/* ── Order row (dark style) ── */
function OrderRow({ item }: { item: WorkQueueItem }) {
  return (
    <div className="flex items-center justify-between gap-3 px-4 py-3 border-b border-[hsl(220,15%,14%)] last:border-0 hover:bg-[hsl(220,15%,13%)] transition-colors">
      <div className="min-w-0 flex-1">
        <div className="flex items-center gap-2">
          ...
          {item.invoice_number && (
            <Link to={`/admin/invoices/${item.invoice_id}`} className="text-[11px] text-emerald-400 hover:underline font-mono">
              #{item.invoice_number}
            </Link>
          )}
        </div>
        <p className="text-[12px] text-[hsl(220,10%,45%)] mt-0.5 truncate">
      ...
      <div className="flex items-center gap-2 shrink-0">
        {item.total_amount != null && (
          <span className="text-[13px] font-medium tabular-nums text-[hsl(220,10%,70%)]">
...
function QueueSection({
  ...
  return (
    <div className="rounded-lg border border-[hsl(220,15%,16%)] bg-[hsl(220,20%,11%)] overflow-hidden">
      <div className="flex items-center justify-between px-4 py-3 border-b border-[hsl(220,15%,16%)]">
        <div className="flex items-center gap-2">
      ...
      {isLoading ? (
        ...
        <p className="text-[12px] text-[hsl(220,10%,35%)] text-center py-6">{emptyMessage}</p>
      ) : (
        children
      )}
    </div>
    ...
    <div className="space-y-5">
      <div className="flex items-center justify-between">
        ...
        <button
          ...
          className="flex items-center gap-1.5 rounded-lg border border-[hsl(220,15%,16%)] bg-[hsl(220,20%,11%)] px-3 py-1.5 text-[12px] text-[hsl(220,10%,50%)] hover:text-white transition-colors"
        >
        ...
        {kpis.map((k) => (
          <div key={k.label} className="rounded-lg border border-[hsl(220,15%,16%)] bg-[hsl(220,20%,11%)] p-3">
            <div className="flex items-center gap-2 mb-1.5">
              ...
              <span className="text-[11px] uppercase tracking-wider text-[hsl(220,10%,40%)]">{k.label}</span>
            </div>
            <p className={`text-xl font-bold tabular-nums ${k.warn ? "text-amber-400" : "text-white"}`}>
              {isLoading ? "—" : k.value}
            </p>
          </div>
        ))}
