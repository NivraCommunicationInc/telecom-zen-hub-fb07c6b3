...
export const SystemStatusBannerBase = ({ userType, supabaseClient }: SystemStatusBannerBaseProps) => {
  ...
  const { data: statuses } = useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await supabaseClient
        .from("system_status")
        .select(
          "id, title, message, status_type, severity, is_banner, starts_at, ends_at, affected_services, show_to_clients, show_to_employees, show_to_technicians"
        )
        .eq("is_active", true)
        .eq("is_banner", true)
  ...
  return (
    <div className="space-y-0 w-full" style={{ width: "100%" }}>
      {activeStatuses.map((status) => {
        ...
        return (
          <div
            key={status.id}
            className={cn(
              "w-full px-4 py-2 flex items-center justify-between gap-4 text-sm",
