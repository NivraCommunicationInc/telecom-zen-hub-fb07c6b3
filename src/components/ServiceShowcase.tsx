        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 lg:gap-8">
          {serviceCards.map((service) => (
            <Link
              key={service.title}
              to={service.link}
              className="group relative bg-white rounded-2xl border border-slate-200/80 p-8 transition-all duration-300 hover:border-blue-200 hover:shadow-lg"
            >
              ...
              <p className="text-slate-500 text-sm mb-6">{service.subtitle}</p>

              {service.startingPrice != null && (
                <div className="mb-6">
                  <span className="text-3xl font-bold text-slate-900">{service.startingPrice.toFixed(0)}$</span>
                  <span className="text-slate-400 ml-1">/{isFr ? "mois" : "mo"}</span>
                </div>
              )}
