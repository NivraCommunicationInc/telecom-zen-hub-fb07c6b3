...
import { Save, Truck, Wrench, Bell, CheckCircle2 } from "lucide-react";
...
export function ShippingTechnicianStep({ proc }: Props) {
  ...
  const handleMarkShipped = async () => {
    await proc.updateShipping({ shipped_at: new Date().toISOString() });
  ...
  const handleInstallComplete = async () => {
    ...
    toast.success("Installation complétée");
  };
      ...
      {isShipping ? (
        <>
          ...
          <div className="flex flex-wrap gap-2 pt-4 border-t border-gray-100">
            <Button size="sm" onClick={handleSaveShipping} disabled={proc.isUpdating} className="text-xs h-8 bg-gray-900 text-white hover:bg-gray-800">
              <Save className="w-3 h-3 mr-1" /> Sauvegarder
            </Button>
            <Button size="sm" onClick={handleMarkShipped} disabled={proc.isUpdating} className="text-xs h-8 bg-emerald-600 hover:bg-emerald-700 text-white">
              <Truck className="w-3 h-3 mr-1" /> Marquer expédié
            </Button>
            <Button size="sm" variant="outline" className="text-xs h-8 border-gray-300 text-gray-700">
              <Bell className="w-3 h-3 mr-1" /> Notifier le client
            </Button>
          </div>
        </>
      ) : (
        <>
          ...
          <div className="flex flex-wrap gap-2 pt-4 border-t border-gray-100">
            <Button size="sm" onClick={handleAssignTechnician} disabled={proc.isUpdating} className="text-xs h-8 bg-gray-900 text-white hover:bg-gray-800">
              <Wrench className="w-3 h-3 mr-1" /> Assigner technicien
            </Button>
            <Button size="sm" onClick={handleInstallComplete} disabled={proc.isUpdating} className="text-xs h-8 bg-emerald-600 hover:bg-emerald-700 text-white">
              <CheckCircle2 className="w-3 h-3 mr-1" /> Installation complétée
            </Button>
          </div>
        </>
      )}
