import React, { useState } from "react";
import { Card, CardContent, CardHeader, CardTitle, CardDescription } from "@/components/ui/card";
import { Button } from "@/components/ui/button";
import { Badge } from "@/components/ui/badge";
import { Input } from "@/components/ui/input";
import { Label } from "@/components/ui/label";
import { Textarea } from "@/components/ui/textarea";
import { Switch } from "@/components/ui/switch";
import {
  Dialog,
  DialogContent,
  DialogHeader,
  DialogTitle,
  DialogDescription,
  DialogFooter,
} from "@/components/ui/dialog";
import {
  AlertDialog,
  AlertDialogAction,
  AlertDialogCancel,
  AlertDialogContent,
  AlertDialogDescription,
  AlertDialogFooter,
  AlertDialogHeader,
  AlertDialogTitle,
} from "@/components/ui/alert-dialog";
import {
  Select,
  SelectContent,
  SelectItem,
  SelectTrigger,
  SelectValue,
} from "@/components/ui/select";
import { ScrollArea } from "@/components/ui/scroll-area";
import { useToast } from "@/hooks/use-toast";
import { useMutation, useQueryClient } from "@tanstack/react-query";
import { useCanonicalClientData } from "@/hooks/useCanonicalClientData";
import { portalClient as supabase } from "@/integrations/backend";
import { useClientAuth } from "@/hooks/useClientAuth";
import { useLanguage } from "@/contexts/LanguageContext";
import {
  Users,
  Plus,
  Edit2,
  Trash2,
  Phone,
  Mail,
  Star,
  Shield,
  Bell,
} from "lucide-react";
import { format } from "date-fns";

interface AuthorizedContact {
  id: string;
  full_name: string;
  relationship_label: string | null;
  phone: string | null;
  email: string | null;
  permission_level: string;
  is_primary: boolean;
  notification_opt_in: boolean;
  created_at: string;
  updated_at: string;
  created_by: string | null;
  created_by_role: string | null;
}

const permissionLevels = {
  level_1: {
    label: "Level 1 — Information Only",
    labelFr: "Niveau 1 — Information seulement",
    description: "Can receive updates and discuss account status",
    descriptionFr: "Peut recevoir des mises à jour et discuter du statut du compte",
    color: "bg-gray-500/20 text-gray-400",
  },
  level_2: {
    label: "Level 2 — Service Requests",
    labelFr: "Niveau 2 — Demandes de service",
    description: "Can request plan changes, appointments, troubleshooting",
    descriptionFr: "Peut demander des changements de forfait, rendez-vous, dépannage",
    color: "bg-blue-500/20 text-blue-400",
  },
  level_3: {
    label: "Level 3 — Billing Assistance",
    labelFr: "Niveau 3 — Aide à la facturation",
    description: "Can discuss invoices, payments, balances",
    descriptionFr: "Peut discuter des factures, paiements, soldes",
    color: "bg-amber-500/20 text-amber-400",
  },
  level_4: {
    label: "Level 4 — Full Representative",
    labelFr: "Niveau 4 — Représentant autorisé",
    description: "Full access as authorized representative",
    descriptionFr: "Accès complet en tant que représentant autorisé",
    color: "bg-emerald-500/20 text-emerald-400",
  },
};

const ClientAuthorizedContacts: React.FC = () => {
  const { user } = useClientAuth();
  const { language } = useLanguage();
  const { toast } = useToast();
  const queryClient = useQueryClient();
  const [dialogOpen, setDialogOpen] = useState(false);
  const [deleteDialogOpen, setDeleteDialogOpen] = useState(false);
  const [editingContact, setEditingContact] = useState<AuthorizedContact | null>(null);
  const [contactToDelete, setContactToDelete] = useState<AuthorizedContact | null>(null);
  const [formData, setFormData] = useState({
    full_name: "",
    relationship_label: "",
    phone: "",
    email: "",
    permission_level: "level_1",
    is_primary: false,
    notification_opt_in: false,
  });

  const t = (en: string, fr: string) => (language === "fr" ? fr : en);

  const { data: contacts = [], isLoading } = useQuery({
    queryKey: ["authorized-contacts", user?.id],
    enabled: !!user?.id,
    queryFn: async () => {
      const { data, error } = await supabase
        .from("authorized_users")
        .select("*")
        .eq("client_id", user!.id)
        .order("is_primary", { ascending: false })
        .order("created_at", { ascending: false });

      if (error) throw error;
      return data as AuthorizedContact[];
    },
  });

  const addContactMutation = useMutation({
    mutationFn: async (data: typeof formData) => {
      // Check for duplicates
      const existingByPhone = contacts.find(
        (c) => c.phone === data.phone && c.id !== editingContact?.id
      );
      const existingByEmail = contacts.find(
        (c) => c.email && c.email === data.email && c.id !== editingContact?.id
      );

      if (existingByPhone) {
        throw new Error(t("A contact with this phone number already exists", "Un contact avec ce numéro existe déjà"));
      }
      if (existingByEmail) {
        throw new Error(t("A contact with this email already exists", "Un contact avec ce courriel existe déjà"));
      }

      if (editingContact) {
        const { error } = await supabase
          .from("authorized_users")
          .update({
            full_name: data.full_name,
            relationship_label: data.relationship_label || null,
            phone: data.phone || null,
            email: data.email || null,
            permission_level: data.permission_level,
            is_primary: data.is_primary,
            notification_opt_in: data.notification_opt_in,
            updated_at: new Date().toISOString(),
          })
          .eq("id", editingContact.id);

        if (error) throw error;
      } else {
        const { error } = await supabase.from("authorized_users").insert({
          client_id: user!.id,
          full_name: data.full_name,
          relationship_label: data.relationship_label || null,
          phone: data.phone || null,
          email: data.email || null,
          permission_level: data.permission_level,
          is_primary: data.is_primary,
          notification_opt_in: data.notification_opt_in,
          created_by: user!.id,
          created_by_role: "client",
        });

        if (error) throw error;
      }
    },
    onSuccess: () => {
      toast({
        title: editingContact
          ? t("Contact updated", "Contact mis à jour")
          : t("Contact added", "Contact ajouté"),
      });
      queryClient.invalidateQueries({ queryKey: ["authorized-contacts"] });
      closeDialog();
    },
    onError: (error: any) => {
      toast({
        title: t("Error", "Erreur"),
        description: error.message,
        variant: "destructive",
      });
    },
  });

  const deleteContactMutation = useMutation({
    mutationFn: async (contactId: string) => {
      const { error } = await supabase
        .from("authorized_users")
        .delete()
        .eq("id", contactId);

      if (error) throw error;
    },
    onSuccess: () => {
      toast({ title: t("Contact removed", "Contact supprimé") });
      queryClient.invalidateQueries({ queryKey: ["authorized-contacts"] });
      setDeleteDialogOpen(false);
      setContactToDelete(null);
    },
    onError: (error: any) => {
      toast({
        title: t("Error", "Erreur"),
        description: error.message,
        variant: "destructive",
      });
    },
  });

  const openAddDialog = () => {
    setEditingContact(null);
    setFormData({
      full_name: "",
      relationship_label: "",
      phone: "",
      email: "",
      permission_level: "level_1",
      is_primary: false,
      notification_opt_in: false,
    });
    setDialogOpen(true);
  };

  const openEditDialog = (contact: AuthorizedContact) => {
    setEditingContact(contact);
    setFormData({
      full_name: contact.full_name,
      relationship_label: contact.relationship_label || "",
      phone: contact.phone || "",
      email: contact.email || "",
      permission_level: contact.permission_level || "level_1",
      is_primary: contact.is_primary || false,
      notification_opt_in: contact.notification_opt_in || false,
    });
    setDialogOpen(true);
  };

  const closeDialog = () => {
    setDialogOpen(false);
    setEditingContact(null);
    setFormData({
      full_name: "",
      relationship_label: "",
      phone: "",
      email: "",
      permission_level: "level_1",
      is_primary: false,
      notification_opt_in: false,
    });
  };

  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    if (!formData.full_name.trim()) {
      toast({
        title: t("Name required", "Nom requis"),
        variant: "destructive",
      });
      return;
    }
    if (!formData.phone.trim()) {
      toast({
        title: t("Phone required", "Téléphone requis"),
        variant: "destructive",
      });
      return;
    }
    addContactMutation.mutate(formData);
  };

  const getPermissionBadge = (level: string) => {
    const config = permissionLevels[level as keyof typeof permissionLevels] || permissionLevels.level_1;
    return (
      <Badge className={config.color}>
        {language === "fr" ? config.labelFr.split(" — ")[1] : config.label.split(" — ")[1]}
      </Badge>
    );
  };

  return (
    <Card>
      <CardHeader>
        <div className="flex items-center justify-between">
          <div>
            <CardTitle className="flex items-center gap-2">
              <Users className="w-5 h-5 text-cyan-500" />
              {t("Authorized Contacts", "Contacts autorisés")}
            </CardTitle>
            <CardDescription>
              {t(
                "People authorized to manage your account",
                "Personnes autorisées à gérer votre compte"
              )}
            </CardDescription>
          </div>
          <Button size="sm" onClick={openAddDialog}>
            <Plus className="w-4 h-4 mr-2" />
            {t("Add Contact", "Ajouter")}
          </Button>
        </div>
      </CardHeader>
      <CardContent>
        {isLoading ? (
          <div className="flex items-center justify-center py-8">
            <div className="w-6 h-6 border-2 border-cyan-500 border-t-transparent rounded-full animate-spin" />
          </div>
        ) : contacts.length === 0 ? (
          <div className="text-center py-8 text-muted-foreground">
            <Users className="w-12 h-12 mx-auto mb-3 opacity-40" />
            <p>{t("No authorized contacts", "Aucun contact autorisé")}</p>
            <p className="text-sm mt-1">
              {t(
                "Add contacts who can manage your account",
                "Ajoutez des contacts pouvant gérer votre compte"
              )}
            </p>
          </div>
        ) : (
          <ScrollArea className="max-h-[400px]">
            <div className="space-y-3">
              {contacts.map((contact) => (
                <div
                  key={contact.id}
                  className="flex items-start justify-between p-4 rounded-lg border bg-card hover:bg-muted/30 transition-colors"
                >
                  <div className="space-y-1">
                    <div className="flex items-center gap-2">
                      <span className="font-medium">{contact.full_name}</span>
                      {contact.is_primary && (
                        <Badge variant="outline" className="text-xs border-amber-500/50 text-amber-500">
                          <Star className="w-3 h-3 mr-1" />
                          {t("Primary", "Principal")}
                        </Badge>
                      )}
                    </div>
                    {contact.relationship_label && (
                      <p className="text-sm text-muted-foreground">{contact.relationship_label}</p>
                    )}
                    <div className="flex flex-wrap items-center gap-3 text-sm text-muted-foreground">
                      {contact.phone && (
                        <span className="flex items-center gap-1">
                          <Phone className="w-3 h-3" />
                          {contact.phone}
                        </span>
                      )}
                      {contact.email && (
                        <span className="flex items-center gap-1">
                          <Mail className="w-3 h-3" />
                          {contact.email}
                        </span>
                      )}
                      {contact.notification_opt_in && (
                        <span className="flex items-center gap-1 text-cyan-500">
                          <Bell className="w-3 h-3" />
                          {t("Notifications", "Notifications")}
                        </span>
                      )}
                    </div>
                    <div className="pt-1">{getPermissionBadge(contact.permission_level)}</div>
                  </div>
                  <div className="flex items-center gap-1">
                    <Button variant="ghost" size="icon" onClick={() => openEditDialog(contact)}>
                      <Edit2 className="w-4 h-4" />
                    </Button>
                    <Button
                      variant="ghost"
                      size="icon"
                      className="text-destructive hover:text-destructive"
                      onClick={() => {
                        setContactToDelete(contact);
                        setDeleteDialogOpen(true);
                      }}
                    >
                      <Trash2 className="w-4 h-4" />
                    </Button>
                  </div>
                </div>
              ))}
            </div>
          </ScrollArea>
        )}
      </CardContent>

      {/* Add/Edit Dialog */}
      <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
        <DialogContent className="max-w-md">
          <DialogHeader>
            <DialogTitle className="flex items-center gap-2">
              <Shield className="w-5 h-5 text-cyan-500" />
              {editingContact
                ? t("Edit Authorized Contact", "Modifier le contact")
                : t("Add Authorized Contact", "Ajouter un contact autorisé")}
            </DialogTitle>
            <DialogDescription>
              {t(
                "This person will be able to manage your account based on their permission level",
                "Cette personne pourra gérer votre compte selon son niveau d'autorisation"
              )}
            </DialogDescription>
          </DialogHeader>

          <form onSubmit={handleSubmit} className="space-y-4">
            <div className="space-y-2">
              <Label>{t("Full Name", "Nom complet")} *</Label>
              <Input
                value={formData.full_name}
                onChange={(e) => setFormData({ ...formData, full_name: e.target.value })}
                placeholder={t("John Doe", "Jean Dupont")}
              />
            </div>

            <div className="space-y-2">
              <Label>{t("Relationship", "Relation")}</Label>
              <Input
                value={formData.relationship_label}
                onChange={(e) => setFormData({ ...formData, relationship_label: e.target.value })}
                placeholder={t("e.g., Spouse, Parent, Guardian", "ex: Conjoint, Parent, Tuteur")}
              />
            </div>

            <div className="grid grid-cols-2 gap-3">
              <div className="space-y-2">
                <Label>{t("Phone", "Téléphone")} *</Label>
                <Input
                  value={formData.phone}
                  onChange={(e) => setFormData({ ...formData, phone: e.target.value })}
                  placeholder="514-555-1234"
                />
              </div>
              <div className="space-y-2">
                <Label>{t("Email", "Courriel")}</Label>
                <Input
                  type="email"
                  value={formData.email}
                  onChange={(e) => setFormData({ ...formData, email: e.target.value })}
                  placeholder="email@example.com"
                />
              </div>
            </div>

            <div className="space-y-2">
              <Label>{t("Permission Level", "Niveau d'autorisation")}</Label>
              <Select
                value={formData.permission_level}
                onValueChange={(value) => setFormData({ ...formData, permission_level: value })}
              >
                <SelectTrigger>
                  <SelectValue />
                </SelectTrigger>
                <SelectContent>
                  {Object.entries(permissionLevels).map(([key, config]) => (
                    <SelectItem key={key} value={key}>
                      <div className="space-y-0.5">
                        <p className="font-medium">
                          {language === "fr" ? config.labelFr : config.label}
                        </p>
                        <p className="text-xs text-muted-foreground">
                          {language === "fr" ? config.descriptionFr : config.description}
                        </p>
                      </div>
                    </SelectItem>
                  ))}
                </SelectContent>
              </Select>
            </div>

            <div className="space-y-3">
              <div className="flex items-center justify-between">
                <div>
                  <Label>{t("Primary Contact", "Contact principal")}</Label>
                  <p className="text-xs text-muted-foreground">
                    {t("First point of contact", "Premier point de contact")}
                  </p>
                </div>
                <Switch
                  checked={formData.is_primary}
                  onCheckedChange={(checked) => setFormData({ ...formData, is_primary: checked })}
                />
              </div>

              <div className="flex items-center justify-between">
                <div>
                  <Label>{t("Receive Notifications", "Recevoir des notifications")}</Label>
                  <p className="text-xs text-muted-foreground">
                    {t("SMS/email updates", "Mises à jour par SMS/courriel")}
                  </p>
                </div>
                <Switch
                  checked={formData.notification_opt_in}
                  onCheckedChange={(checked) =>
                    setFormData({ ...formData, notification_opt_in: checked })
                  }
                />
              </div>
            </div>

            <DialogFooter>
              <Button type="button" variant="outline" onClick={closeDialog}>
                {t("Cancel", "Annuler")}
              </Button>
              <Button type="submit" disabled={addContactMutation.isPending}>
                {addContactMutation.isPending
                  ? t("Saving...", "Enregistrement...")
                  : editingContact
                  ? t("Update", "Mettre à jour")
                  : t("Add Contact", "Ajouter")}
              </Button>
            </DialogFooter>
          </form>
        </DialogContent>
      </Dialog>

      {/* Delete Confirmation */}
      <AlertDialog open={deleteDialogOpen} onOpenChange={setDeleteDialogOpen}>
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>
              {t("Remove Authorized Contact?", "Supprimer le contact autorisé?")}
            </AlertDialogTitle>
            <AlertDialogDescription>
              {t(
                `"${contactToDelete?.full_name}" will no longer be able to access your account.`,
                `« ${contactToDelete?.full_name} » ne pourra plus accéder à votre compte.`
              )}
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
            <AlertDialogCancel>{t("Cancel", "Annuler")}</AlertDialogCancel>
            <AlertDialogAction
              className="bg-destructive hover:bg-destructive/90"
              onClick={() => contactToDelete && deleteContactMutation.mutate(contactToDelete.id)}
              disabled={deleteContactMutation.isPending}
            >
              {deleteContactMutation.isPending ? t("Removing...", "Suppression...") : t("Remove", "Supprimer")}
            </AlertDialogAction>
          </AlertDialogFooter>
        </AlertDialogContent>
      </AlertDialog>
    </Card>
  );
};

export default ClientAuthorizedContacts;