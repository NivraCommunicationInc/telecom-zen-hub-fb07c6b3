...
export default function CoverageSection() {
  ...
  return (
    <section aria-label={isFr ? "Zones de couverture" : "Coverage areas"} className="py-16 px-6 bg-[#0a0a0a]">
      <div className="max-w-[1100px] mx-auto text-center">
        ...
        <div className="flex flex-wrap justify-center gap-2 mb-10">
          {regions.map((r) => (
            <span
              key={r}
              className="px-3 py-1.5 text-sm bg-white/5 border border-white/10 rounded-full text-white/70 hover:border-cyan-500/30 hover:text-cyan-400 transition-colors"
            >
              {r}
            </span>
          ))}
