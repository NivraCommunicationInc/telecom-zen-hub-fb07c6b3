...
const Footer = forwardRef<HTMLElement>((_, ref) => {
  ...
  return (
    <footer ref={ref} className="bg-slate-900 text-white border-t border-slate-800" data-testid="footer">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-2 lg:grid-cols-4 gap-10 lg:gap-12">
          ...
          <div className="col-span-2 lg:col-span-1">
            ...
            <div className="flex items-center gap-3 mt-5">
              <a href="https://www.facebook.com/profile.php?id=61584408712750" target="_blank" rel="noopener noreferrer" className="text-slate-500 hover:text-white transition-colors" aria-label="Facebook">
                <Facebook className="w-5 h-5" />
              </a>
              <a href="https://www.instagram.com/nivratelecom/" target="_blank" rel="noopener noreferrer" className="text-slate-500 hover:text-white transition-colors" aria-label="Instagram">
          ...
          {/* Services */}
          <div>
            <h4 className="font-semibold text-xs uppercase tracking-widest mb-5 text-slate-400">Services</h4>
            <ul className="space-y-3">
              {links.services.map((link) => (
                <li key={link.href}>
                  <Link to={link.href} className="text-slate-500 hover:text-white transition-colors text-sm">{link.label}</Link>
                </li>
              ))}
            </ul>
          </div>
          ...
          {/* Support */}
          <div>
            <h4 className="font-semibold text-xs uppercase tracking-widest mb-5 text-slate-400">Support</h4>
            <ul className="space-y-3">
          ...
          {/* Legal */}
          <div>
            <h4 className="font-semibold text-xs uppercase tracking-widest mb-5 text-slate-400">{isFr ? "Légal" : "Legal"}</h4>
            <ul className="space-y-3">
        ...
        <div className="container mx-auto px-4 sm:px-6 py-5 max-w-[1320px]">
          <div className="flex flex-col sm:flex-row items-center justify-between gap-4">
            <div className="flex flex-col items-center sm:items-start gap-2">
              <p className="text-slate-500 text-xs uppercase tracking-wider font-medium">{isFr ? "Paiement" : "Payment"}</p>
            ...
            <div className="flex flex-col items-center sm:items-end gap-2">
              <p className="text-slate-500 text-xs uppercase tracking-wider font-medium">{isFr ? "Sécurité" : "Security"}</p>
      ...
      <div className="border-t border-slate-800">
        <div className="container mx-auto px-4 sm:px-6 py-4 max-w-[1320px]">
          <div className="flex flex-col items-center gap-2 text-center sm:flex-row sm:justify-between sm:text-left">
            <p className="text-slate-500 text-xs">© {currentYear} {COMPANY_CONTACT.legalName}</p>
            <div className="flex flex-wrap items-center justify-center gap-x-3 gap-y-1">
              <Link to="/privacy-policy" className="text-slate-500 hover:text-white transition-colors text-xs">{isFr ? "Confidentialité" : "Privacy"}</Link>
              <span className="text-slate-700 text-xs">·</span>
              <Link to="/terms-and-conditions" className="text-slate-500 hover:text-white transition-colors text-xs">{isFr ? "Conditions" : "Terms"}</Link>
              <span className="text-slate-700 text-xs">·</span>
