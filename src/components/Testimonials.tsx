...
const fallback: Testimonial[] = [
  ...
  { name: "Pierre D.", location: "Terrebonne", text: "Très satisfait du service Internet. Les prix sont clairs, pas de surprise sur la facture.", rating: 5 },
  { name: "Caroline B.", location: "Longueuil", text: "J'apprécie de pouvoir tout gérer en ligne. Le portail est simple à utiliser.", rating: 4 },
];
...
const Testimonials = () => {
  ...
  useEffect(() => {
    let mounted = true;
    (async () => {
      const { data, error } = await supabase.rpc("get_featured_testimonials" as any);
      if (!mounted) return;
      if (error || !data || (Array.isArray(data) && data.length === 0)) {
        setItems(fallback);
      } else {
        const mapped: Testimonial[] = (data as any[]).map((r) => ({
          name: r.client_name ?? "Client",
          location: r.client_city ?? "",
          text: r.comment ?? "",
    ...
    <section className="section-padding bg-muted/30">
      <div className="container mx-auto px-4 max-w-6xl">
        <div className="text-center mb-10">
          ...
          <p className="text-muted-foreground">Ce que nos clients disent de nos services</p>
        </div>
        ...
        {loading ? (
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
            {(items ?? fallback).map((testimonial, index) => (
              <div key={index} className="bg-card border border-border rounded-2xl p-6 hover:border-accent/30 transition-colors">
                <div className="flex items-center gap-1 mb-4">
                  {Array.from({ length: 5 }).map((_, i) => (
                    <Star key={i} className={`w-4 h-4 ${i < testimonial.rating ? "text-amber-400 fill-amber-400" : "text-muted-foreground"}`} />
                  ))}
                </div>
                <div className="relative mb-4">
                  <Quote className="w-8 h-8 text-accent/20 absolute -top-2 -left-1" />
                  <p className="text-foreground text-sm leading-relaxed pl-6">{testimonial.text}</p>
                </div>
                <div className="flex items-center gap-3 pt-4 border-t border-border">
                  <div className="w-10 h-10 rounded-full bg-accent/10 flex items-center justify-center">
                    <span className="text-accent font-semibold text-sm">{testimonial.name.charAt(0)}</span>
                  </div>
                  <div>
