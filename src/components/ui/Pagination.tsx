/**
 * Pagination — Reusable paginator with ellipsis.
 * Dark-theme friendly with violet accent. Max 7 page buttons visible.
 */
...
export function Pagination({
  ...
  const ellipsisCls = cn("min-w-[28px] h-10 inline-flex items-center justify-center text-sm",
  ...
  return (
    <nav
      ...
      className={cn("flex flex-col sm:flex-row items-center justify-between gap-3 pt-2", className)}
    >
      ...
      <div className="flex items-center gap-1 flex-wrap justify-center">
        ...
        {range.map((p, idx) =>
          p === "…" ? (
            <span key={`e-${idx}`} className={ellipsisCls}>…</span>
          ) : (
            <button
              key={p}
              type="button"
