  ...
  return (
    <div>
      ...
      <div className="px-4 py-5 space-y-4">
        <section className="rounded-2xl bg-violet-600/10 border border-violet-600/30 p-5 text-center">
          <ScanLine className="h-12 w-12 text-violet-400 mx-auto mb-3" />
        ...
        <div className="space-y-3">
          <input
            ...
            className="w-full min-h-[56px] rounded-full bg-slate-900 border border-slate-800 text-white px-5 text-base focus:outline-none focus:ring-2 focus:ring-violet-500"
            onKeyDown={(e) => e.key === "Enter" && lookup()}
          />
          <button
            onClick={lookup}
            disabled={loading || !code.trim()}
            className="w-full min-h-[56px] rounded-full bg-violet-600 hover:bg-violet-500 text-white text-base font-semibold flex items-center justify-center gap-2 disabled:opacity-40"
          >
        ...
        {result && (
          <section className="rounded-2xl bg-slate-900 border border-slate-800 p-4 space-y-2">
            ...
            <p className="text-xs">
              <span className="inline-block px-2 py-1 rounded-full bg-violet-600/20 text-violet-300 font-medium uppercase">
                {result.status}
              </span>
            </p>
          </section>
