/**
 * TechBottomNav — High-vis operations rail.
 * Tabs: Dashboard · RDV · Scanner · Carte · Menu
 * Uses design tokens from tech-portal.css.
 */
import { NavLink } from "react-router-dom";
import { Gauge, CalendarClock, Map, ScanLine, LayoutGrid } from "lucide-react";
...
export default function TechBottomNav() {
  ...
  const tabs: Tab[] = [
    { to: "/tech", icon: Gauge, label: "Dash", end: true },
    { to: "/tech/appointments", icon: CalendarClock, label: "RDV", badge: available.length, urgent: urgentCount > 0 },
  ...
  return (
    ...
    >
      <ul className="flex items-end px-2 pt-2 pb-1 max-w-[760px] mx-auto">
        {tabs.map((tab) => {
                ...
                {({ isActive }) =>
                  center ? (
                    <>
                      ...
                      <span
                        className="tp-italic-label text-[10px] leading-none"
