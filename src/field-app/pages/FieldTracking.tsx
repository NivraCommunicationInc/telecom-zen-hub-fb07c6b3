/**
 * FieldTracking — Sales pipeline tracking with operational states.
 * Tracks field_sales_orders (real orders) alongside leads.
 */
...
export default function FieldTracking() {
  ...
  const { data, isLoading } = useQuery({
    queryKey: ["field-tracking-v2", user?.id],
    queryFn: async () => {
      const [leadsRes, ordersRes] = await Promise.all([
        supabase.from("field_leads").select("status").eq("agent_id", user!.id),
        supabase.from("field_sales_orders").select("payment_status, sync_status").eq("salesperson_id", user!.id),
      ]);
    ...
    <div className="space-y-6">
      ...
      {isLoading ? (
        ...
        <div className="text-center py-12">
          ...
          <p className="text-sm text-muted-foreground">Aucune donnée de vente</p>
        </div>
      ) : (
        <>
          {/* KPI Cards */}
          <div className="grid grid-cols-3 gap-3">
            <div className="bg-card border border-border rounded-xl p-4 text-center">
              <p className="text-[11px] text-muted-foreground font-medium">Commandes</p>
              <p className="text-2xl font-bold text-foreground mt-1">{totalOrders}</p>
            </div>
            <div className="bg-card border border-border rounded-xl p-4 text-center">
              <p className="text-[11px] text-muted-foreground font-medium">Payées</p>
              <p className="text-2xl font-bold text-emerald-600 mt-1">{data?.paymentCounts?.confirmed || 0}</p>
            </div>
            <div className="bg-card border border-border rounded-xl p-4 text-center">
              <p className="text-[11px] text-muted-foreground font-medium">Sync Core</p>
              <p className="text-2xl font-bold text-emerald-600 mt-1">{data?.syncCounts?.synced || 0}</p>
            </div>
          </div>
            ...
            <div>
              ...
              <div className="space-y-2">
                {ORDER_PIPELINE.map((stage) => {
                  const count = data?.paymentCounts?.[stage.key] || 0;
                  const pct = totalOrders > 0 ? (count / totalOrders) * 100 : 0;
                  return (
                    <div key={stage.key} className="p-3 rounded-xl border border-border bg-card">
          ...
          {totalOrders > 0 && (
            <div>
              ...
              <div className="space-y-2">
                {SYNC_PIPELINE.map((stage) => {
                  const count = data?.syncCounts?.[stage.key] || 0;
                  const pct = totalOrders > 0 ? (count / totalOrders) * 100 : 0;
                  return (
                    <div key={`sync-${stage.key}`} className="p-3 rounded-xl border border-border bg-card">
                      <div className="flex items-center justify-between mb-1.5">
                        <span className="text-sm font-medium text-foreground">{stage.label}</span>
                        <span className="text-lg font-bold text-foreground">{count}</span>
                      </div>
            ...
            <div>
              ...
              <div className="space-y-2">
                {LEAD_PIPELINE.map((stage) => {
                  const count = data?.leadCounts?.[stage.key] || 0;
