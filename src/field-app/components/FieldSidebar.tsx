/**
 * FieldSidebar — Navigation using backend service layer for badges.
 */
...
function NavSection({ title, items, isActive, badges }: { title: string; items: { label: string; href: string; icon: any; badgeKey?: string }[]; isActive: (h: string) => boolean; badges: Record<string, number> }) {
  return (
    <div className="space-y-0.5">
      <p className="px-3 pt-3 pb-1 text-[10px] font-bold text-muted-foreground uppercase tracking-widest">{title}</p>
      {items.map((item) => {
        const badge = item.badgeKey ? badges[item.badgeKey] : 0;
        return (
          <Link key={item.href} to={item.href}
            className={cn("flex items-center justify-between px-2.5 py-2 rounded-lg text-[13px] font-medium transition-colors mx-1",
      ...
      <aside className="hidden md:flex flex-col w-60 border-r border-border bg-card">
        <div className="h-14 flex items-center px-4 border-b border-border">
          <Link to={`${FIELD_BASE}/dashboard`} className="flex items-center gap-2.5">
            ...
            <div>
              <span className="font-bold text-sm text-foreground tracking-tight block leading-tight">Nivra Field</span>
              <span className="text-[9px] text-muted-foreground font-medium">Portail terrain</span>
            </div>
          </Link>
        </div>
        ...
        <ScrollArea className="flex-1 py-1">
          <div className="px-1 space-y-0.5 pt-2">
            {mainNav.map((item) => (
              <Link key={item.href} to={item.href}
                className={cn("flex items-center gap-2.5 px-2.5 py-2 rounded-lg text-[13px] font-medium transition-colors mx-1",
                  item.highlight && !isActive(item.href) ? "text-primary bg-primary/5 hover:bg-primary/10" :
                  isActive(item.href) ? "bg-primary/10 text-primary" : "text-muted-foreground hover:text-foreground hover:bg-secondary"
                )}>
        ...
        <div className="border-t border-border py-2 px-2 space-y-0.5">
          ...
          <button onClick={handleLogout}
            className="w-full flex items-center gap-2.5 px-2.5 py-2 rounded-lg text-[13px] font-medium text-muted-foreground hover:text-destructive hover:bg-destructive/5 transition-colors">
            <LogOut className="h-4 w-4 shrink-0" /><span>Déconnexion</span>
          </button>
        </div>
      </aside>
      ...
      <nav className="md:hidden fixed bottom-0 left-0 right-0 z-50 border-t border-border bg-card px-1 py-1 safe-bottom">
        <div className="flex items-center justify-around">
          {[
            ...
            <Link key={item.href} to={item.href}
              className={cn("flex flex-col items-center gap-0.5 py-1.5 px-2 rounded-lg text-[10px] font-medium transition-colors",
                item.primary ? "text-primary-foreground bg-primary rounded-full px-4 py-2 -mt-3 shadow-lg" :
