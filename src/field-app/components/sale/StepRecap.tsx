...
import { ArrowLeft, ArrowRight, Receipt, TrendingUp, User, MapPin } from "lucide-react";
...
export default function StepRecap({
  ...
  // Live commission preview (best-effort — uses commission_rules)
    ...
    <div className="space-y-5 field-page-enter">
      <div>
        ...
        <p className="text-sm text-[hsl(var(--field-text-muted))] mt-1">
          Vérifiez les détails avant de générer le paiement.
        </p>
      </div>

      {/* Client card */}
      <div className="rounded-2xl border border-[hsl(var(--field-border-subtle))] bg-[hsl(var(--field-card))] p-4">
        ...
        {draft.customer.address && (
          <div className="flex items-start gap-1.5 text-xs text-[hsl(var(--field-text-dim))] mt-2 pt-2 border-t border-[hsl(var(--field-border-subtle))]">
            ...
            <span>
              {draft.customer.address}, {draft.customer.city}, {draft.customer.province} {draft.customer.postal_code}
            </span>
          </div>
        )}
      </div>

      {/* Services + Equipment breakdown */}
      <div className="rounded-2xl border border-[hsl(var(--field-border-subtle))] bg-[hsl(var(--field-card))] p-4 space-y-3">
        ...
        {draft.services.length > 0 && (
          <div className="space-y-1.5">
            <p className="text-[10px] uppercase tracking-wider text-[hsl(var(--field-text-dim))]">Forfaits récurrents</p>
            {draft.services.map((s) => (
              <div key={s.id} className="flex justify-between text-sm">
                <span className="text-white truncate pr-2">{s.name}</span>
                <span className="text-white flex-shrink-0">{formatCAD(s.monthlyPrice)}/mois</span>
              </div>
            ))}
          </div>
        )}
        ...
        {draft.equipment.length > 0 && (
          <div className="space-y-1.5 pt-2 border-t border-[hsl(var(--field-border-subtle))]">
            <p className="text-[10px] uppercase tracking-wider text-[hsl(var(--field-text-dim))]">Équipement</p>
            {draft.equipment.map((e) => (
              <div key={e.id} className="flex justify-between text-sm">
                <span className="text-white truncate pr-2">
                  {e.name} {e.quantity > 1 && `×${e.quantity}`}
                </span>
                <span className="text-white flex-shrink-0">{formatCAD(e.price * e.quantity)}</span>
              </div>
            ))}
          </div>
        )}
        ...
        {draft.discount && (
          <div className="pt-2 border-t border-[hsl(var(--field-border-subtle))]">
            <div className="flex justify-between text-sm">
              <span className="text-[hsl(var(--field-success))]">Rabais — {draft.discount.name}</span>
        ...
        <div className="pt-2 border-t border-[hsl(var(--field-border-subtle))] space-y-1.5">
          <div className="flex justify-between text-sm">
            <span className="text-[hsl(var(--field-text-muted))]">
              Frais d'activation ({draft.services.length} {draft.services.length > 1 ? "services" : "service"})
            </span>
          ...
          <div className="flex justify-between text-xs">
            <span className="text-[hsl(var(--field-text-muted))]">TPS ({(TPS_RATE * 100).toFixed(0)} %)</span>
            <span className="text-[hsl(var(--field-text-muted))]">{formatCAD(tps)}</span>
          </div>
          <div className="flex justify-between text-xs">
            <span className="text-[hsl(var(--field-text-muted))]">TVQ ({(TVQ_RATE * 100).toFixed(3)} %)</span>
        ...
        <div className="pt-3 border-t border-[hsl(var(--field-accent)/0.3)] flex items-center justify-between">
          <span className="text-sm uppercase tracking-wider text-[hsl(var(--field-text-muted))]">Total</span>
          <span className="text-2xl font-bold text-[hsl(var(--field-accent-glow))]">{formatCAD(total)}</span>
        </div>
      </div>

      {/* Commission preview */}
      {commissionRate > 0 && (
        <div className="rounded-2xl border border-[hsl(var(--field-accent)/0.3)] field-gradient-purple p-4">
