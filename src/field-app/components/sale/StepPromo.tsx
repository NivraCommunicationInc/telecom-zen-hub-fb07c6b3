/**
 * Step 2.5 — Promotion / Discount selection
 * Uses backend catalog for approved promos instead of direct DB queries.
 */
...
export default function StepPromo({ selectedPromos, monthlySubtotal, activationFee, onChange, onNext, onBack }: Props) {
  ...
  // Fetch promos from the backend catalog engine
  ...
  // Merge both field_promotions and promotions from catalog
  const promos: FieldSalePromo[] = [
    ...(catalogData?.field_promotions || []).map((fp: any) => ({
      id: fp.id,
      name: fp.name,
      description: fp.description,
      promo_type: fp.promo_type || "monthly_discount",
      discount_monthly: Number(fp.discount_monthly) || 0,
      discount_onetime: Number(fp.discount_onetime) || 0,
      discount_percentage: Number(fp.discount_percentage) || 0,
      duration_months: Number(fp.duration_months) || 1,
      requires_approval: fp.requires_approval ?? false,
    })),
    ...(catalogData?.promotions || [])
      .filter((p: any) => p.status === "active")
      .map((p: any) => ({
        id: p.id,
        name: p.name || p.promo_code,
        description: p.description,
        promo_type: p.promo_type || p.discount_mode || "monthly_discount",
        discount_monthly: 0,
        discount_onetime: 0,
  ...
  const addManualDiscount = () => {
    ...
    setManualName(""); setManualAmount(""); setManualDuration("1"); setManualReason("");
    setShowManual(false);
  };
  ...
  const totalOnetimeDiscount = selectedPromos.reduce((sum, p) => {
    if (["free_installation", "activation_credit"].includes(p.promo_type)) return sum + p.discount_onetime;
    ...
    <div className="space-y-6">
              ...
              return (
                <button
                  ...
                  className={cn(
                    ...
                    active ? "border-[#22C55E] bg-[#F0FDF4]" : "border-[#E5E7EB] bg-white hover:border-[#D1D5DB]"
                  )}
                >
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-3 flex-1 min-w-0">
                      <div className={cn("h-10 w-10 rounded-lg flex items-center justify-center shrink-0", active ? "bg-[#DCFCE7]" : "bg-[#FEF3C7]")}>
                      ...
                      <div className="min-w-0">
                        <div className="flex items-center gap-2">
                          ...
                          {promo.requires_approval && (
                            <span className="text-[10px] font-medium px-1.5 py-0.5 rounded bg-[#FEF3C7] text-[#D97706]">Approbation requise</span>
                          )}
                        </div>
                        {promo.description && <p className="text-xs text-[#6B7280] mt-0.5">{promo.description}</p>}
                        <p className="text-xs font-medium text-[#DC2626] mt-1">{promoImpactLabel(promo, monthlySubtotal)}</p>
                      </div>
                    </div>
                    <div className={cn("h-6 w-6 rounded-full border-2 flex items-center justify-center transition-colors shrink-0 ml-3", active ? "bg-[#22C55E] border-[#22C55E]" : "border-[#D1D5DB]")}>
      ...
      <div>
        <div className="flex items-center justify-between mb-2">
          <p className="text-xs font-semibold text-[#374151] uppercase tracking-wider">Rabais manuel</p>
          {!showManual && (
            <button type="button" onClick={() => setShowManual(true)} className="flex items-center gap-1.5 text-xs font-medium text-[#22C55E] hover:text-[#16A34A] transition-colors">
              <PlusCircle className="h-4 w-4" /> Ajouter un rabais
            </button>
          )}
        </div>
        ...
        {showManual && (
          <div className="border-2 border-[#E5E7EB] rounded-xl p-4 space-y-3 bg-[#FAFAFA]">
            <div>
              ...
              <input type="text" value={manualName} onChange={(e) => setManualName(e.target.value)} placeholder="Ex: Rabais fidélité client"
            ...
            <div className="grid grid-cols-2 gap-3">
              <div>
                ...
                <select value={manualType} onChange={(e) => setManualType(e.target.value as "monthly" | "onetime")}
              ...
              <div>
                ...
                <input type="number" step="0.01" min="0.01" value={manualAmount} onChange={(e) => setManualAmount(e.target.value)} placeholder="10.00"
            ...
            {manualType === "monthly" && (
              <div>
                ...
                <input type="number" min="1" max="24" value={manualDuration} onChange={(e) => setManualDuration(e.target.value)}
            ...
            <div>
              ...
              <input type="text" value={manualReason} onChange={(e) => setManualReason(e.target.value)} placeholder="Ex: Client référé par un partenaire"
                className="w-full px-3 py-2 rounded-lg border border-[#D1D5DB] text-sm text-[#000000] placeholder:text-[#9CA3AF] focus:outline-none focus:ring-2 focus:ring-[#22C55E]" />
            </div>
            <div className="flex gap-2">
              <button type="button" onClick={() => setShowManual(false)} className="flex-1 py-2 rounded-lg border border-[#E5E7EB] text-sm font-medium text-[#374151] hover:bg-[#F3F4F6] transition-colors">Annuler</button>
              <button type="button" onClick={addManualDiscount} disabled={!manualName.trim() || !manualAmount || parseFloat(manualAmount) <= 0}
                className="flex-1 py-2 rounded-lg bg-[#22C55E] text-white text-sm font-semibold hover:bg-[#16A34A] transition-colors disabled:opacity-40 disabled:cursor-not-allowed">Ajouter le rabais</button>
            </div>
        ...
        {manualSelected.length > 0 && (
          <div className="space-y-2 mt-2">
            {manualSelected.map((promo) => (
              <div key={promo.id} className="flex items-center justify-between p-3 rounded-xl border-2 border-[#22C55E] bg-[#F0FDF4]">
                ...
                <button type="button" onClick={() => removePromo(promo.id)} className="h-8 w-8 rounded-lg flex items-center justify-center text-[#DC2626] hover:bg-[#FEF2F2] transition-colors shrink-0 ml-2">
      ...
      {selectedPromos.length > 0 && (
        <div className="bg-[#FEF2F2] border border-[#FECACA] rounded-xl p-4 space-y-2">
          <p className="text-xs font-semibold text-[#DC2626] uppercase tracking-wider">Impact des promotions</p>
