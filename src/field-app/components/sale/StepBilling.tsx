 ...
 */
import { CreditCard, RotateCcw, Check } from "lucide-react";
...
export default function StepBilling({ services, equipment, billing, onChange, onNext, onBack }: Props) {
  ...
  const monthlyTaxes = estimateTaxes(monthlySubtotal);
      ...
      <div className="bg-white border border-[#E5E7EB] rounded-xl p-5 space-y-3">
        ...
        <div className="border-t border-[#E5E7EB] pt-2 space-y-1">
          ...
          <div className="flex justify-between text-xs text-[#6B7280]">
            ...
            <span>{monthlyTaxes.tps.toFixed(2)} $</span>
          </div>
          <div className="flex justify-between text-xs text-[#6B7280]">
            <span>{TAX_DISPLAY.TVQ_LABEL}</span>
            <span>{monthlyTaxes.tvq.toFixed(2)} $</span>
          </div>
          <div className="flex justify-between text-sm font-bold text-[#000000] pt-1">
            <span>Total mensuel</span>
            <span>{monthlyTaxes.total.toFixed(2)} $/mois</span>
          </div>
        </div>
      </div>
      ...
      <div className="bg-[#000000] text-white rounded-xl p-5">
        ...
        <div className="space-y-1 text-sm">
          <div className="flex justify-between">
            <span className="text-[#D1D5DB]">Premier mois</span>
            <span>{monthlySubtotal.toFixed(2)} $</span>
          </div>
          <div className="flex justify-between">
            <span className="text-[#D1D5DB]">Frais uniques</span>
        ...
        <div className="flex items-center justify-between">
          ...
          <button
            ...
            className={cn(
              ...
              billing.preauthorizedPayment ? "bg-[#22C55E]" : "bg-[#D1D5DB]"
            )}
          >
            <div className={cn(
              "h-5 w-5 bg-white rounded-full shadow absolute top-0.5 transition-transform",
              billing.preauthorizedPayment ? "translate-x-5" : "translate-x-0.5"
            )} />
          </button>
        </div>
