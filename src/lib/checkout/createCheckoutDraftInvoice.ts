...
export async function createCheckoutDraftInvoice(
  ...
  const { data: newOrder, error: orderErr } = await supabase
    ...
    .insert([{
      ...
      client_full_address: [input.serviceAddress, input.serviceCity, "QC", input.servicePostalCode].filter(Boolean).join(", "),
      shipping_address: input.serviceAddress,
      shipping_city: input.serviceCity,
      shipping_postal_code: input.servicePostalCode,
