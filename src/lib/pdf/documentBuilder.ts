/**
 * Nivra Document Builder — Order→PDF Pipeline V3
 * 
 * Takes an order ID and generates all 4 required documents:
 ...
 * All totals come from DB. PDF templates PRINT ONLY, never recalculate.
 */
...
export async function fetchOrderDocumentData(orderId: string): Promise<OrderDocumentData | null> {
  ...
  // Fetch profile, account, billing invoice, payments, contract in parallel
  const [profileRes, accountRes, invoiceRes, paymentsRes, contractRes] = await Promise.all([
    ...
    supabase.from("billing_payments").select("*").eq("invoice_id", order.id).order("created_at", { ascending: false }),
...
function parseLineItems(order: any): {
  ...
  const equipment: Array<{ name: string; quantity: number; unit_price: number; serial?: string }> = [];
  ...
  if (eqDetails?.line_items && Array.isArray(eqDetails.line_items)) {
    eqDetails.line_items.forEach((item: any) => {
      ...
      if (item.type === "service" || cat === "Internet" || cat === "Mobile" || cat === "TV" || cat === "Streaming") {
        services.push({ type: cat, name, description: item.description, monthly_price: price });
        invoiceItems.push({
          category: cat as any, description: name, qty, unit_price: price,
          amount: price * qty, is_recurring: true, service_address: order.shipping_address || undefined,
        });
      } else if (item.type === "equipment") {
        equipment.push({ name, quantity: qty, unit_price: price, serial: item.serial });
        invoiceItems.push({
          category: "Equipment", description: name, qty, unit_price: price,
          amount: price * qty, is_recurring: false, reference: item.serial,
        });
      } else if (item.type === "fee") {
        fees.push({ label: name, amount: price });
        invoiceItems.push({
          category: "Fees", description: name, qty: 1, unit_price: price,
          amount: price, is_recurring: false,
        });
      } else {
        invoiceItems.push({
          category: cat as any, description: name, qty, unit_price: price,
          amount: price * qty, is_recurring: false,
        });
      }
    });
  }

  // Fallback: create items from order fields
...
export function buildInvoiceData(data: OrderDocumentData): InvoiceDataV2 {
  const { order, profile, account, billingInvoice } = data;
  const addr = buildCustomerAddress(order, profile, account);
  const { invoiceItems } = parseLineItems(order);

  // TOTALS: prefer billing_invoices (DB source of truth), fallback to calculated
  let subtotal: number, tpsAmount: number, tvqAmount: number, total: number, amountPaid: number;

  if (billingInvoice) {
    subtotal = Number(billingInvoice.subtotal || 0);
    tpsAmount = Number(billingInvoice.tps_amount || 0);
    tvqAmount = Number(billingInvoice.tvq_amount || 0);
    total = Number(billingInvoice.total || 0);
    amountPaid = Number(billingInvoice.amount_paid || 0);
  } else {
    subtotal = invoiceItems.reduce((sum, item) => sum + item.amount, 0);
    const discountAmount = Number(order.discount_amount || 0);
    const taxableBase = Math.max(0, subtotal - discountAmount);
    tpsAmount = Number(order.tps_amount) || Math.round(taxableBase * TAX.GST_RATE * 100) / 100;
    tvqAmount = Number(order.tvq_amount) || Math.round(taxableBase * TAX.QST_RATE * 100) / 100;
    total = Math.round((taxableBase + tpsAmount + tvqAmount) * 100) / 100;
  ...
  const invoiceStatus = billingInvoice?.status || 
    (order.payment_status === "captured" || order.payment_status === "paid" || order.payment_status === "confirmed" ? "paid" : "unpaid");
  ...
  const invoiceData: InvoiceDataV2 = {
    ...
    customer: {
      full_name: requireField(clientName, "client_name"),
      email: requireField(order.client_email || profile?.email, "client_email"),
      phone: requireField(order.client_phone || profile?.phone, "client_phone"),
      address_line1: requireField(addr.address_line1, "address_line1"),
      city: requireField(addr.city, "city"),
      province: addr.province || "QC",
      postal_code: requireField(addr.postal_code, "postal_code"),
    },
...
export function buildOrderSummaryData(data: OrderDocumentData): OrderSummaryV3Data {
  ...
  return {
    ...
    tax_gst: taxGst,
    tax_qst: taxQst,
    total_due: Math.round((taxableBase + taxGst + taxQst) * 100) / 100,
...
export function buildContractData(data: OrderDocumentData): ContractDataV3 {
  const { order, profile, account, contract } = data;
  const addr = buildCustomerAddress(order, profile, account);
  const { services, equipment, fees } = parseLineItems(order);
  ...
  return {
    ...
    signature_name: buildClientName(order, profile), // Use client name as signature
...
export function buildContractSummaryData(data: OrderDocumentData): ContractSummaryData {
  const { order, profile, account, contract } = data;
  const addr = buildCustomerAddress(order, profile, account);
  const { services, fees } = parseLineItems(order);

  const clientName = buildClientName(order, profile);

  const subtotalMonthly = services.reduce((sum, s) => sum + s.monthly_price, 0);
  const subtotalOneTime = fees.reduce((sum, f) => sum + f.amount, 0);
  const discountAmount = Number(order.discount_amount || 0);
  const taxableBase = Math.max(0, subtotalMonthly + subtotalOneTime - discountAmount);
  const taxGst = Number(order.tps_amount) || Math.round(taxableBase * TAX.GST_RATE * 100) / 100;
  const taxQst = Number(order.tvq_amount) || Math.round(taxableBase * TAX.QST_RATE * 100) / 100;
  ...
  return {
    ...
    tax_qst: taxQst,
    total_due_today: Math.round((taxableBase + taxGst + taxQst) * 100) / 100,
    payment_method: requireField(order.payment_method, "payment_method"),
