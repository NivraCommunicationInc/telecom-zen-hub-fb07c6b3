...
// CVV is salted+hashed with SHA-256 (PBKDF2-style, 100k iterations) â€” bcrypt npm shim
...
async function encryptCard(plain: string): Promise<{ ciphertext: string; iv: string; tag: string }> {
  ...
  // WebCrypto AES-GCM concatenates ciphertext|tag â€” split last 16 bytes as tag.
