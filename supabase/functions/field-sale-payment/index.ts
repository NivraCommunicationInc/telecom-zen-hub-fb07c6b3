...
serve(async (req) => {
  ...
  try {
    ...
    const { action, amount, customer_email, customer_name, description, lead_id } = body;
        ...
        line_items: [
          {
            price_data: {
              currency: "cad",
              product_data: {
                name: description || "Nivra — Commande terrain",
                description: `Réf: ${lead_id || "N/A"}`,
              },
    ...
    if (action === "create_payment_intent") {
      // Create PaymentIntent for in-person card payment (immediate capture)
      const paymentIntent = await stripe.paymentIntents.create({
        amount: amountCents,
        currency: "cad",
        capture_method: "automatic",
        description: description || "Nivra — Paiement terrain",
