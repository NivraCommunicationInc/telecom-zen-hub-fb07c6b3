...
Deno.serve(async (req) => {
  ...
  try {
    ...
    console.log(`[marketing-send-sms-${reqId}] to(raw)=${to} → normalized=${normalizedTo}`);
    if (!normalizedTo) {
      return new Response(JSON.stringify({ error: "Invalid phone number format", raw: to }), { status: 400, headers: { ...corsHeaders, "Content-Type": "application/json" } });
    }
    ...
    let pnJson: any = null;
    try { pnJson = JSON.parse(pnText); } catch { /* keep raw */ }
    console.log(`[marketing-send-sms-${reqId}] phone-numbers status=${pnRes.status} body=${pnText.slice(0, 500)}`);

    if (!pnRes.ok) {
      return new Response(JSON.stringify({ error: "Failed to fetch OpenPhone numbers", status: pnRes.status, details: pnJson || pnText }), { status: 500, headers: { ...corsHeaders, "Content-Type": "application/json" } });
    }
    ...
    console.log(`[marketing-send-sms-${reqId}] from candidate phoneNumber=${fromNumber} id=${fromId}`);

    if (!fromNumber && !fromId) {
      return new Response(JSON.stringify({ error: "No OpenPhone number available", pn_payload: pnJson }), { status: 500, headers: { ...corsHeaders, "Content-Type": "application/json" } });
    }
    ...
    console.log(`[marketing-send-sms-${reqId}] POST /v1/messages body=${JSON.stringify(sendBody)}`);
    ...
    try { sendJson = JSON.parse(sendText); } catch { /* keep raw */ }
    console.log(`[marketing-send-sms-${reqId}] OpenPhone send status=${sendRes.status} body=${sendText.slice(0, 1000)}`);

    if (!sendRes.ok) {
      // If `from` as ID failed, retry with E.164 number (some accounts require this)
      if (fromId && fromNumber && fromValue === fromId) {
        console.log(`[marketing-send-sms-${reqId}] retry with from=E.164 ${fromNumber}`);
        const retryBody = { content: message, from: fromNumber, to: [normalizedTo] };
        ...
        console.log(`[marketing-send-sms-${reqId}] retry status=${retryRes.status} body=${retryText.slice(0, 1000)}`);
        if (retryRes.ok) {
          // success on retry — fall through with retryJson
          return await finishSuccess(admin, retryJson, normalizedTo, message, user, conversation_id, corsHeaders);
        }
        return new Response(JSON.stringify({ error: "OpenPhone send failed", status: retryRes.status, attempts: { primary: sendJson || sendText, retry: retryJson || retryText } }), { status: 500, headers: { ...corsHeaders, "Content-Type": "application/json" } });
      }
      return new Response(JSON.stringify({ error: "OpenPhone send failed", status: sendRes.status, details: sendJson || sendText }), { status: 500, headers: { ...corsHeaders, "Content-Type": "application/json" } });
    }

    return await finishSuccess(admin, sendJson, normalizedTo, message, user, conversation_id, corsHeaders);
  } catch (e) {
...
async function finishSuccess(admin: any, sendJson: any, normalizedTo: string, message: string, user: any, conversation_id: string | null, corsHeaders: Record<string, string>) {
  // Resolve / upsert conversation
  let convId = conversation_id || null;
  if (!convId) {
    const { data: existing } = await admin
      .from("marketing_conversations").select("id").eq("phone_number", normalizedTo).maybeSingle();
    if (existing) convId = existing.id;
    else {
      const { data: created } = await admin
        .from("marketing_conversations")
        .insert({ phone_number: normalizedTo, status: "human_takeover", ai_enabled: false })
        .select("id").single();
