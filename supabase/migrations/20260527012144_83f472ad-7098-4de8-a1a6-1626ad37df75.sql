...
BEGIN
  ...
  WHERE o.user_id = ANY(coalesce(v_related_user_ids, ARRAY[]::uuid[]))
     ...
     OR EXISTS (
       ...
       WHERE lp.order_id = o.id
         AND (
           lp.user_id = ANY(coalesce(v_related_user_ids, ARRAY[]::uuid[]))
           OR lp.client_id = ANY(coalesce(v_related_user_ids, ARRAY[]::uuid[]))
           OR (coalesce(array_length(v_account_ids, 1), 0) > 0 AND lp.account_id = ANY(v_account_ids))
         )
     )
     OR EXISTS (
       SELECT 1 FROM public.phone_orders po
       WHERE po.order_id = o.id
         AND (
           po.user_id = ANY(coalesce(v_related_user_ids, ARRAY[]::uuid[]))
           OR (coalesce(array_length(v_account_ids, 1), 0) > 0 AND po.account_id = ANY(v_account_ids))
         )
     );
  ...
  FROM (
    ...
    WHERE (si.user_id = ANY(coalesce(v_related_user_ids, ARRAY[]::uuid[])) OR (coalesce(array_length(v_order_ids, 1), 0) > 0 AND si.order_id = ANY(v_order_ids)))
      AND si.account_id IS NOT NULL
    UNION
    SELECT po.account_id
    FROM public.phone_orders po
    WHERE (po.user_id = ANY(coalesce(v_related_user_ids, ARRAY[]::uuid[])) OR (coalesce(array_length(v_order_ids, 1), 0) > 0 AND po.order_id = ANY(v_order_ids)))
      AND po.account_id IS NOT NULL
  ) resolved_accounts;
  ...
  FROM (
    ...
    WHERE mi.client_id = ANY(coalesce(v_related_user_ids, ARRAY[]::uuid[]))
    ORDER BY mi.id, mi.created_at DESC NULLS LAST
  ) src;
  ...
  FROM (
    ...
    WHERE p.user_id = ANY(coalesce(v_related_user_ids, ARRAY[]::uuid[]))
       OR p.client_id = ANY(coalesce(v_related_user_ids, ARRAY[]::uuid[]))
       OR (coalesce(array_length(v_account_ids, 1), 0) > 0 AND p.account_id = ANY(v_account_ids))
       OR (coalesce(array_length(v_order_ids, 1), 0) > 0 AND p.order_id = ANY(v_order_ids))
    ORDER BY p.id, p.created_at DESC NULLS LAST
  ) src;
  ...
  FROM (
    ...
    WHERE c.owner_user_id = ANY(coalesce(v_related_user_ids, ARRAY[]::uuid[]))
       OR c.user_id = ANY(coalesce(v_related_user_ids, ARRAY[]::uuid[]))
  ...
  FROM (
    ...
    WHERE d.client_id = ANY(coalesce(v_related_user_ids, ARRAY[]::uuid[]))
  ...
  FROM (
    ...
    WHERE d.user_id = ANY(coalesce(v_related_user_ids, ARRAY[]::uuid[]))
    ORDER BY d.id, d.created_at DESC NULLS LAST
  ) src;
  ...
  FROM (
    ...
    FROM public.service_instances si
    WHERE si.user_id = ANY(coalesce(v_related_user_ids, ARRAY[]::uuid[]))
  ...
  FROM (
    SELECT DISTINCT ON (sa.id) sa.*
    FROM public.service_addresses sa
    WHERE coalesce(array_length(v_account_ids, 1), 0) > 0
      AND sa.account_id = ANY(v_account_ids)
    ORDER BY sa.id, sa.is_primary DESC NULLS LAST, sa.created_at DESC NULLS LAST
  ) src;
  ...
  INTO v_phone_orders
  FROM (
    SELECT DISTINCT ON (po.id) po.*
    FROM public.phone_orders po
    WHERE po.user_id = ANY(coalesce(v_related_user_ids, ARRAY[]::uuid[]))
       OR (coalesce(array_length(v_account_ids, 1), 0) > 0 AND po.account_id = ANY(v_account_ids))
       OR (coalesce(array_length(v_order_ids, 1), 0) > 0 AND po.order_id = ANY(v_order_ids))
    ORDER BY po.id, po.created_at DESC NULLS LAST
  ) src;

  SELECT coalesce(jsonb_agg(to_jsonb(src) ORDER BY src.created_at DESC NULLS LAST), '[]'::jsonb)
  INTO v_payment_proofs
  FROM (
    SELECT DISTINCT ON (pp.id) pp.*
    FROM public.payment_proofs pp
    WHERE pp.client_id = ANY(coalesce(v_related_user_ids, ARRAY[]::uuid[]))
       OR EXISTS (
         SELECT 1 FROM public.billing_payments bp
         WHERE bp.id = pp.payment_id
           AND (
             (coalesce(array_length(v_customer_ids, 1), 0) > 0 AND bp.customer_id = ANY(v_customer_ids))
             OR EXISTS (
               SELECT 1 FROM public.billing_invoices bi
               WHERE bi.id = bp.invoice_id
                 AND (
    ...
    ORDER BY pp.id, pp.created_at DESC NULLS LAST
  ) src;
  ...
  FROM (
    SELECT * FROM (
      SELECT cal.id, cal.created_at, 'client_activity_logs'::text AS source, cal.action_type::text AS action, cal.entity_type::text, cal.entity_id, cal.actor_name, cal.actor_role, cal.summary::text AS summary, cal.before_data, cal.after_data, NULL::jsonb AS details
      FROM public.client_activity_logs cal
      WHERE cal.client_id = ANY(coalesce(v_related_user_ids, ARRAY[]::uuid[]))
         OR (coalesce(array_length(v_order_ids, 1), 0) > 0 AND cal.entity_id = ANY(v_order_ids))
         OR (coalesce(array_length(v_subscription_ids, 1), 0) > 0 AND cal.entity_id = ANY(v_subscription_ids))
         OR (coalesce(array_length(v_equipment_ids, 1), 0) > 0 AND cal.entity_id = ANY(v_equipment_ids))
      UNION ALL
      SELECT al.id, al.created_at, 'activity_logs'::text AS source, al.action::text AS action, al.entity_type::text, al.entity_id, al.actor_name, al.actor_role, coalesce(al.reason, al.changed_field, al.action)::text AS summary, NULL::jsonb AS before_data, NULL::jsonb AS after_data, al.details
      FROM public.activity_logs al
      WHERE al.user_id = ANY(coalesce(v_related_user_ids, ARRAY[]::uuid[]))
         OR (coalesce(array_length(v_order_ids, 1), 0) > 0 AND al.entity_id = ANY(v_order_ids))
         OR (coalesce(array_length(v_subscription_ids, 1), 0) > 0 AND al.entity_id = ANY(v_subscription_ids))
      UNION ALL
      SELECT osh.id, osh.created_at, 'order_status_history'::text AS source, coalesce(osh.new_status, 'status_change')::text AS action, 'order'::text AS entity_type, osh.order_id AS entity_id, osh.actor_name, osh.actor_role, osh.change_reason::text AS summary, jsonb_build_object('status', osh.old_status) AS before_data, jsonb_build_object('status', osh.new_status) AS after_data, osh.metadata AS details
      FROM public.order_status_history osh
      WHERE coalesce(array_length(v_order_ids, 1), 0) > 0 AND osh.order_id = ANY(v_order_ids)
      UNION ALL
      SELECT eal.id, eal.created_at, 'equipment_audit_log'::text AS source, eal.action::text AS action, 'equipment'::text AS entity_type, eal.equipment_id AS entity_id, eal.actor_name, NULL::text AS actor_role, eal.action::text AS summary, jsonb_build_object('status', eal.old_status) AS before_data, jsonb_build_object('status', eal.new_status) AS after_data, eal.details
      FROM public.equipment_audit_log eal
      WHERE coalesce(array_length(v_equipment_ids, 1), 0) > 0 AND eal.equipment_id = ANY(v_equipment_ids)
      UNION ALL
      SELECT bsta.id, bsta.created_at, 'billing_subscription_trace_audit'::text AS source, bsta.action::text AS action, 'subscription'::text AS entity_type, bsta.subscription_id AS entity_id, NULL::text AS actor_name, NULL::text AS actor_role, bsta.reason::text AS summary, NULL::jsonb AS before_data, NULL::jsonb AS after_data, bsta.details
      FROM public.billing_subscription_trace_audit bsta
      WHERE (coalesce(array_length(v_subscription_ids, 1), 0) > 0 AND bsta.subscription_id = ANY(v_subscription_ids))
         OR (coalesce(array_length(v_customer_ids, 1), 0) > 0 AND bsta.customer_id = ANY(v_customer_ids))
      UNION ALL
      SELECT oal.id, oal.created_at, 'order_automation_log'::text AS source, oal.action::text AS action, oal.entity_type::text, coalesce(oal.entity_id, oal.order_id) AS entity_id, NULL::text AS actor_name, 'system'::text AS actor_role, oal.action::text AS summary, NULL::jsonb AS before_data, NULL::jsonb AS after_data, oal.details
      FROM public.order_automation_log oal
      WHERE coalesce(array_length(v_order_ids, 1), 0) > 0 AND oal.order_id = ANY(v_order_ids)
    ) unioned
