...
-- ============================================================
-- SYSTEMIC FIX: Prevent order completion without paid invoice
-- ============================================================
...
DECLARE
  ...
  v_alert_reason text;
BEGIN
  -- Only guard transitions INTO completion states
  ...
  IF v_invoice.status NOT IN ('paid', 'partially_paid') AND COALESCE(v_invoice.balance_due, v_invoice.total) > 0 THEN
    ...
    VALUES (
      ...
      jsonb_build_object(
        ...
        'reason', 'Invoice exists but is not paid. Completion blocked.',
        'attempted_status', NEW.status,
        'timestamp', now()
      )
    );
