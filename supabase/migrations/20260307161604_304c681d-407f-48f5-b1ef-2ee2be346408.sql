...
-- Fix: Replace 'system' text fallback with NULL for confirmed_by UUID column
...
BEGIN
  ...
  INSERT INTO public.channel_selections (
    ...
    CASE WHEN v_status = 'confirmed' THEN NEW.channel_assigned_by ELSE NULL END,
    now()
  )
...
BEGIN
  ...
  IF NEW.status IN ('activated', 'completed', 'installation_completed', 'delivered') THEN
    ...
    SET
      ...
      confirmed_by = COALESCE(confirmed_by, NEW.channel_assigned_by),
      updated_at = now()
