...
BEGIN
  ...
  WITH acct AS (
    SELECT a.*, lower(btrim(p.email)) AS account_email
  ...
  account_customers AS (
    ...
    JOIN acct a ON a.client_id = bc.user_id OR (a.account_email IS NOT NULL AND lower(btrim(bc.email)) = a.account_email)
  ),
  account_orders AS (
    SELECT o.*
    FROM public.orders o
    JOIN acct a ON (o.account_id = a.id OR o.user_id = a.client_id OR (a.account_email IS NOT NULL AND lower(btrim(o.client_email)) = a.account_email))
  ),
  ...
  account_appointments AS (
    ...
    WHERE ap.client_id = a.client_id
       ...
       OR (a.account_email IS NOT NULL AND lower(btrim(ap.client_email)) = a.account_email)
  ),
  address_nodes AS (
    SELECT
      sa.created_at,
      jsonb_build_object(
        ...
        'tickets', COALESCE((
          ...
          JOIN acct a ON true
          WHERE (t.account_id = _account_id OR t.user_id = a.client_id OR t.related_order_id IN (SELECT id FROM account_orders) OR (a.account_email IS NOT NULL AND lower(btrim(t.client_email)) = a.account_email))
