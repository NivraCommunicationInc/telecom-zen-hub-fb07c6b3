...
-- =====================================================
-- generate_account_renewal_invoice(p_account_id uuid)
-- Generates a single unified renewal invoice for ALL active
-- subscriptions on an account, anchored to billing_cycle_day.
-- Promos NEVER apply to renewal invoices (type='renewal').
-- Updates accounts.next_invoice_date after generation.
-- =====================================================
...
BEGIN
  ...
  IF v_cycle_start IS NULL THEN
    -- Fallback: use billing_cycle_day in current month
  ...
  v_due_date  := v_cycle_start; -- prepaid: due ON the cycle start

  -- 4. Check idempotency: already generated for this cycle?
  IF EXISTS (
  ...
  -- 6. Gather all active subscriptions for this customer
  -- and calculate subtotal from their services
  FOR v_sub IN
    SELECT bs.*
  ...
  LOOP
    ...
    -- If no services found, use plan_price
  ...
  -- 7. Calculate taxes (server-side only)
  ...
  -- 9. Create invoice lines per active service
  v_line_count := 0;
  FOR v_sub IN
    SELECT bs.*
    FROM billing_subscriptions bs
