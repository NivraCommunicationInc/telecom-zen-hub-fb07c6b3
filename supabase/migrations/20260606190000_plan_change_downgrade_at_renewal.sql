-- ============================================================
-- Plan change fixes:
-- 1. Add requested_plan_price to service_change_requests so
--    generate_account_renewal_invoice can apply downgrades without
--    a separate services lookup.
-- 2. Update generate_account_renewal_invoice to apply pending
--    downgrades before generating the renewal invoice.
-- ============================================================

ALTER TABLE public.service_change_requests
  ADD COLUMN IF NOT EXISTS requested_plan_price numeric(10,2);

-- Backfill from services table where possible
UPDATE public.service_change_requests scr
SET requested_plan_price = s.price
FROM public.services s
WHERE s.id = scr.requested_plan_id
  AND scr.requested_plan_price IS NULL;

-- ============================================================
-- Updated generate_account_renewal_invoice
-- Adds: apply pending downgrade before invoice generation
-- ============================================================
...
BEGIN
  ...
  -- ============================================================
  -- Apply pending downgrades before computing the invoice amount
  -- ============================================================
  FOR v_downgrade IN
    SELECT scr.*
  ...
  LOOP
    DECLARE
      ...
      v_new_name  text;
    BEGIN
      v_new_name  := v_downgrade.requested_plan_name;
      v_new_price := v_downgrade.requested_plan_price;

      -- Fallback: look up price from services table
      IF v_new_price IS NULL AND v_downgrade.requested_plan_id IS NOT NULL THEN
        SELECT price INTO v_new_price FROM services WHERE id = v_downgrade.requested_plan_id;
      END IF;

      IF v_new_price IS NOT NULL AND v_new_price > 0 THEN
        -- Apply to billing_subscriptions (drives renewal invoice)
        UPDATE billing_subscriptions
        SET plan_name  = v_new_name,
            plan_price = v_new_price,
            updated_at = v_now
        WHERE customer_id = v_customer_id
          AND status = 'active';

        -- Apply to legacy subscriptions table
        UPDATE subscriptions
        SET plan_name     = v_new_name,
            monthly_price = v_new_price,
            amount        = v_new_price,
            updated_at    = v_now
        WHERE user_id = v_account.client_id
          AND status   = 'active';

        -- Mark applied
        UPDATE service_change_requests
        SET status         = 'applied',
            effective_date = v_now,
            updated_at     = v_now
        WHERE id = v_downgrade.id;

        RAISE NOTICE '[renewal] Applied downgrade for account %: % → % ($%)',
          p_account_id, v_downgrade.current_plan_name, v_new_name, v_new_price;
      ELSE
        -- Can't determine price — mark for manual review
        UPDATE service_change_requests
        SET status     = 'pending_manual',
            notes      = COALESCE(notes, '') || ' [renewal: price unknown, manual review needed]',
            updated_at = v_now
        WHERE id = v_downgrade.id;
      END IF;
    END;
