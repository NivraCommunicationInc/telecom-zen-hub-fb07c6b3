...
-- Line-driven subscription provisioning.
-- Each recurring line in the order (is_recurring flag / period = monthly / recurring category)
-- produces its own subscription with plan_name, plan_price, plan_code, cycle taken
-- from the line itself. Equipment, one-time fees, taxes, discounts, delivery, activation
-- and accessories are excluded categorically.
...
BEGIN
  ...
  -- Collect candidate lines from the order (canonical source of truth).
  ...
  -- Also fold in invoice service lines (already normalized upstream) so callers that
  -- only wrote to billing_invoice_lines still get subscriptions.
  ...
  -- Filter down to strictly recurring lines. Reject equipment / fees / taxes / etc.
  ...
  LOOP
    ...
    v_item_is_recurring := COALESCE((v_item->>'is_recurring')::boolean, false);

    -- Hard exclusions: never a subscription regardless of other flags.
    ...
    -- Inclusion: explicit is_recurring flag, OR monthly period, OR service category.
    IF NOT (
      v_item_is_recurring
      OR v_item_period IN ('monthly','month','recurring','mensuel','30_days','30d')
  ...
  -- Iterate each recurring line and upsert a subscription that reflects it exactly.
  ...
  LOOP
    ...
    v_item_code := COALESCE(v_item->>'code', v_item->>'sku', v_item->>'plan_code', v_item->>'ref_id', lower(regexp_replace(v_item_name, '[^a-zA-Z0-9]+', '_', 'g')));
    ...
    v_item_cycle_days := COALESCE((v_item->>'cycle_days')::int,
      CASE lower(v_item_period)
        ...
        WHEN '30d' THEN 30
        WHEN 'recurring' THEN 30
        ELSE 30
      END);

    -- Derive a service_category label directly from the line
    v_item_service_category := CASE
      WHEN v_item_category ILIKE '%internet%' OR lower(v_item_name) ILIKE '%internet%' OR lower(v_item_name) ILIKE '%fibre%' OR lower(v_item_name) ILIKE '%giga%' THEN 'Internet'
      WHEN v_item_category ILIKE '%tv%' OR v_item_category ILIKE '%tele%' OR lower(v_item_name) ILIKE '%tv%' OR lower(v_item_name) ILIKE '%télé%' THEN 'TV'
      WHEN v_item_category ILIKE '%mobile%' OR v_item_category ILIKE '%cell%' OR lower(v_item_name) ILIKE '%mobile%' OR lower(v_item_name) ILIKE '%forfait%' THEN 'Mobile'
      WHEN v_item_category ILIKE '%streaming%' OR lower(v_item_name) ILIKE '%streaming%' OR lower(v_item_name) ILIKE '%netflix%' OR lower(v_item_name) ILIKE '%crave%' OR lower(v_item_name) ILIKE '%dazn%' OR lower(v_item_name) ILIKE '%apple%' THEN 'Streaming'
      WHEN v_item_category ILIKE '%security%' OR lower(v_item_name) ILIKE '%sécurité%' OR lower(v_item_name) ILIKE '%security%' THEN 'Security'
      WHEN v_item_category ILIKE '%combo%' THEN 'TV'
      ELSE COALESCE(NULLIF(v_item->>'service_category', ''), initcap(v_item_category))
    END;

    v_needs_address := lower(v_item_service_category) IN ('internet','tv');

    -- Resolve address if needed (only once per order)
    IF v_needs_address AND v_address_id IS NULL THEN
      ...
      -- Guard: no duplicate active service of same category at same address (other orders)
    ...
    -- Upsert subscription for THIS line (keyed by order_id + plan_code)
    ...
    ELSE
      -- Overwrite with the exact values from the line (line is truth)
    ...
    -- Ensure exactly one recurring row in billing_subscription_services matches the line
    IF NOT EXISTS (
