...
-- Fix search_clients_unified to use correct column names (client_phone not phone)
...
BEGIN
  ...
  WITH all_sources AS (
    ...
    WHERE (norm_email = '' OR lower(trim(COALESCE(o.client_email, ''))) LIKE '%' || norm_email || '%')
      ...
      AND (norm_phone = '' OR regexp_replace(COALESCE(o.client_phone, ''), '[^0-9]', '', 'g') LIKE '%' || norm_phone || '%')
  ),
