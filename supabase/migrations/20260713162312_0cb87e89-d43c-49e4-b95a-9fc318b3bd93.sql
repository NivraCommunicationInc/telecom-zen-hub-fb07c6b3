...
BEGIN
  ...
  SELECT COUNT(*) INTO v_intents FROM public.field_payment_intents
   WHERE converted_order_id = p_order_id
     AND lower(coalesce(status,'')) NOT IN ('captured','cancelled','failed','paid');
...
BEGIN
  ...
  UPDATE public.field_quotes SET status = 'cancelled', updated_at = now()
   WHERE converted_order_id = p_order_id AND lower(coalesce(status,'')) NOT IN ('cancelled','rejected','expired');
