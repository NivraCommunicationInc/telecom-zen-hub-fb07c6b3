...
BEGIN
  ...
  IF v_order_id IS NOT NULL THEN
    SELECT id INTO v_sub_id
     ...
     WHERE order_id = v_order_id AND customer_id = p_customer_id
  ...
  IF v_user_id IS NULL THEN
    RAISE EXCEPTION 'billing_customer % sans user_id : abonnement ad-hoc requiert un user_id lié', p_customer_id USING ERRCODE = 'invalid_parameter_value';
  END IF;
  SELECT id INTO v_account_id FROM public.accounts WHERE client_id = v_user_id ORDER BY created_at DESC LIMIT 1;
  IF v_account_id IS NULL THEN
    RAISE EXCEPTION 'accounts introuvable pour user % : créez le compte avant l''abonnement', v_user_id USING ERRCODE = 'no_data_found';
  END IF;

  v_service_type := CASE lower(COALESCE(p_service_category,''))
    WHEN 'internet' THEN 'internet' WHEN 'tv' THEN 'tv' WHEN 'mobile' THEN 'mobile'
    WHEN 'streaming' THEN 'streaming' WHEN 'security' THEN 'security'
    WHEN 'addon' THEN 'addon' WHEN 'equipment' THEN 'equipment' ELSE 'addon' END;
