...
-- ============================================================================
-- MIGRATE WELCOME DISCOUNT TO SERVER-SIDE compute_checkout_pricing RPC
-- ============================================================================
-- This replaces the client-side useWelcomeDiscount logic with server-side
-- new customer detection and automatic 50% recurring discount.
-- ============================================================================
...
BEGIN
  ...
  -- ============================================================
  -- WELCOME DISCOUNT: 50% on recurring services for new customers
  -- Server-side detection — no client input accepted
  -- ============================================================
  ...
  -- Apply promo if provided (AFTER welcome discount check — no stacking)
  IF p_promo_code IS NOT NULL AND p_promo_code != '' THEN
    -- If welcome discount already applied, skip promo to prevent double discount
    IF v_welcome_applied THEN
      -- No promo stacking with welcome discount
  ...
  -- Total discount = promo + welcome (both already capped individually)
  -- Taxable base = subtotals - all discounts (never negative)
  v_taxable_base := GREATEST(0, v_recurring_subtotal + v_one_time_subtotal - v_discount_total - v_welcome_discount - v_preauth_cents);
  ...
  RETURN jsonb_build_object(
    ...
    'discount_total', ROUND((v_discount_total + v_welcome_discount)::NUMERIC / 100, 2),
    'promo_discount', ROUND(v_discount_total::NUMERIC / 100, 2),
    ...
    'cents', jsonb_build_object(
      ...
      'discount_total', v_discount_total + v_welcome_discount,
      'promo_discount', v_discount_total,
