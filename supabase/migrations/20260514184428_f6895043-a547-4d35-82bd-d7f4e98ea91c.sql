-- Public RPC: fetch a field payment intent + its quote for the /payer/:intentId page.
-- Anonymous-readable but limited to the rows belonging to the requested intent id.
...
DECLARE
  ...
  v_agent_name text;
BEGIN
  SELECT id, quote_id, paypal_order_id, paypal_approval_url, amount, currency,
...
REVOKE ALL ON FUNCTION public.get_field_payment_intent_public(uuid) FROM PUBLIC;
