...
BEGIN
  ...
  BEGIN
    ...
    INSERT INTO public.billing_invoices (
      ...
      now()::date, 'test', v_order_id, 'subscription', now()
  ...
  BEGIN
    ...
    v_result := public.rpc_admin_pause_subscription(
      p_subscription_id := v_sub_id, p_action := 'pause',
  ...
  BEGIN
    ...
    SELECT status INTO v_status_before FROM public.billing_subscriptions WHERE id = v_sub_id;
    v_result := public.rpc_admin_pause_subscription(
      p_subscription_id := v_sub_id, p_action := 'resume',
  ...
  BEGIN
    v_count := public.rpc_apply_referral_discount(
      p_customer_id := v_bc_id, p_order_id := v_order_id,
      p_code := 'QA-P64-REF', p_amount := 5.00, p_months_remaining := 3);
