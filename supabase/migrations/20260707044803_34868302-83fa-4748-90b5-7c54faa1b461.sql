...
BEGIN
  IF p_customer_id IS NULL THEN
    RAISE EXCEPTION 'p_customer_id requis' USING ERRCODE = 'invalid_parameter_value'; END IF;
  IF p_lines IS NULL OR jsonb_typeof(p_lines) <> 'array' OR jsonb_array_length(p_lines) = 0 THEN
    RAISE EXCEPTION 'p_lines doit être un tableau JSON non vide' USING ERRCODE = 'invalid_parameter_value'; END IF;

  -- Hérite automatiquement de l'order_id de l'abonnement pour garder la chaîne complète
  IF v_order_id IS NULL AND p_subscription_id IS NOT NULL THEN
    SELECT order_id INTO v_order_id FROM public.billing_subscriptions WHERE id = p_subscription_id;
  END IF;

  IF v_order_id IS NOT NULL THEN
    SELECT id INTO v_invoice_id FROM public.billing_invoices
      WHERE order_id = v_order_id AND status NOT IN ('void','cancelled') LIMIT 1;
    IF v_invoice_id IS NOT NULL THEN RETURN v_invoice_id; END IF;
  END IF;

  FOR v_line IN SELECT * FROM jsonb_array_elements(p_lines) LOOP
    v_subtotal := v_subtotal + ROUND(COALESCE((v_line->>'line_total')::numeric, 0), 2);
  END LOOP;
  ...
  BEGIN SELECT public.generate_billing_invoice_number() INTO v_invoice_number;
  EXCEPTION WHEN undefined_function THEN v_invoice_number := NULL; END;
  ...
  INSERT INTO public.billing_invoices (
    ...
    p_customer_id, p_subscription_id, v_order_id, v_invoice_number,
  ...
  FOR v_line IN SELECT * FROM jsonb_array_elements(p_lines) LOOP
    v_line_type := COALESCE(v_line->>'line_type', 'service');
    v_source_ref := CASE
      WHEN v_line ? 'source_ref' AND (v_line->>'source_ref') IN
        ('legacy','order_item','manual_admin','credit_application','payment_application','tax','promotion_applied','system_migration')
        THEN v_line->>'source_ref'
      WHEN v_line_type = 'credit'   THEN 'credit_application'
      WHEN v_line_type = 'discount' THEN 'promotion_applied'
      ELSE 'manual_admin'
    END;
  ...
  PERFORM public._nivra_record_provenance(
    ...
    CASE WHEN v_order_id IS NOT NULL THEN 'order' ELSE 'billing_subscription' END,
    COALESCE(v_order_id, p_subscription_id),
    jsonb_build_object('subtotal',v_subtotal,'total',v_total,'line_count',jsonb_array_length(p_lines))
  );
  RETURN v_invoice_id;
END; $$;