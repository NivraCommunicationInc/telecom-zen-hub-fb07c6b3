...
WHERE bss.subscription_id = bs.id
  ...
  AND (
    public.fn_is_equipment_label(bss.service_name)
    OR (bss.service_type NOT IN ('recurring', 'one_time') AND bss.service_name = COALESCE(NULLIF(btrim(o.pricing_snapshot->>'plan_name'), ''), NULLIF(btrim(o.service_type), ''), bss.service_name))
  )
...
BEGIN
  ...
  IF v_has_profile AND nullif(btrim(v_profile.email), '') IS NOT NULL THEN
  ...
  RETURN jsonb_build_object(
    ...
    'identifiers', jsonb_build_object(
      ...
      'profileEmail', CASE WHEN v_has_profile THEN v_profile.email ELSE NULL END,
