...
BEGIN
  ...
  SELECT coalesce(jsonb_agg(to_jsonb(ar) ORDER BY coalesce(ar.submitted_at, ar.updated_at, ar.activated_at, ar.completed_at) DESC NULLS LAST), '[]'::jsonb)
  INTO v_activation_requests
  FROM public.activation_requests ar
  WHERE ar.client_id = ANY(coalesce(v_related_user_ids, ARRAY[]::uuid[]))
