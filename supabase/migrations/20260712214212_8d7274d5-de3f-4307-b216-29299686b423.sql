...
BEGIN
  ...
  -- Créer en draft (sans tax_snapshot) pour que les promotions puissent recalculer les totaux
  BEGIN
  ...
  -- ── Promotions récurrentes (Lot 1) : applique et décrémente months_remaining
  PERFORM public.apply_active_account_promotions_to_invoice(v_invoice_id);

  -- Finaliser: pose tax_snapshot + passe à pending (déclenche le gel des totaux)
  ...
  SET status = 'pending',
      tax_snapshot = jsonb_build_object(
        ...
        'promotions_snapshot', COALESCE(tax_snapshot->'promotions_applied_total', '0'::jsonb)
      )
