CREATE OR REPLACE FUNCTION public.rpc_account_journal_write(p_target_table text, p_payload jsonb, p_event_key text DEFAULT NULL::text, p_correlation_id uuid DEFAULT NULL::uuid)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
...
BEGIN
  ...
  ELSIF p_target_table = 'client_internal_notes' THEN
    INSERT INTO public.client_internal_notes(
      client_id, author_id, author_name, author_role,
      note_type, subject, body, is_pinned, is_urgent, tags,
      ...
      COALESCE((v_payload->>'is_pinned')::boolean, false),
      COALESCE((v_payload->>'is_urgent')::boolean, false),
      COALESCE(ARRAY(SELECT jsonb_array_elements_text(COALESCE(v_payload->'tags','[]'::jsonb))), '{}'::text[]),
  ...
  ELSIF p_target_table = 'order_internal_notes' THEN
    INSERT INTO public.order_internal_notes(
      order_id, author_id, author_name, author_role, body,
      correlation_id, event_key
    ) VALUES (
      v_order_id, v_actor_user_id, v_actor_name, v_actor_role,
      COALESCE(v_payload->>'body',''),
      v_correlation_id, v_event_key
    ) RETURNING id INTO v_new_id;
  END IF;
