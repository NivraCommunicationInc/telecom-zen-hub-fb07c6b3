...
DECLARE
  ...
  v_target_has_client_account boolean;
  v_recent_count int;
...
BEGIN
  ...
  SELECT EXISTS (
    SELECT 1 FROM public.accounts a WHERE a.client_id = _client_id
    UNION ALL
    SELECT 1 FROM public.billing_customers bc WHERE bc.user_id = _client_id
  ) INTO v_target_has_client_account;
  ...
  IF v_target_is_staff AND NOT v_target_has_client_account THEN
    INSERT INTO public.admin_security_audit (admin_user_id, action, target_type, target_id, ip_address, user_agent, details)
    VALUES (v_admin_id, 'client_impersonation_denied', 'profile', _client_id, _ip, _ua,
      jsonb_build_object('reason', 'staff_without_client_account'));
    RAISE EXCEPTION 'Ce profil interne n’a pas de compte client à ouvrir' USING ERRCODE = '42501';
  END IF;

  IF NOT v_target_has_client_account THEN
    INSERT INTO public.admin_security_audit (admin_user_id, action, target_type, target_id, ip_address, user_agent, details)
    VALUES (v_admin_id, 'client_impersonation_denied', 'profile', _client_id, _ip, _ua,
      jsonb_build_object('reason', 'missing_client_account'));
  ...
  VALUES (v_admin_id, 'client_impersonation_started', 'profile', _client_id, _ip, _ua,
    jsonb_build_object('session_id', v_session.id, 'reason', _reason, 'target_had_staff_role', v_target_is_staff));
